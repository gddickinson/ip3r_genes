...
of kb". Ensembl's canonical transcripts say otherwise ({fig:review_gene_architecture}).
...
which ({fig:review_taxonomic_range}). Chapter 5 chases all of them
individually.
...
rather than on a residue list taken from a paper ({fig:review_alignment_windows}).

![](figures/review_conservation.png)

**{fig:review_conservation}.** Per-column conservation across a
...
**{fig:review_regulation}.** The curated regulator map: what binds the
...
resolves through the database's other symbol endpoint [R160], which works,
