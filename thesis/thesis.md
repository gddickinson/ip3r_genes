...
Figure 1.1 introduces the object at the three scales the rest
...
**Figure 1.1.** The receptor is shown at the three scales this
...
the dimensions measured on PDB 6DQN [1]. About nine tenths of each subunit
...
across the split between protostomes and deuterostomes [2], and the
...
Figure 1.2 lays that history out in two panels. Panel **a**
...
![](figures/fig_1.2_discovery_timeline.png)

**Figure 1.2.** The timeline draws four decades of work on the
...
between the two is the machine that couples them. Figure 1.3
...
![](figures/fig_1.3_channel_structure.png)

**Figure 1.3.** The channel is measured here rather than
drawn. (**a**) One C4-symmetric subunit's Cα trace from PDB 6DQN, a human
IP₃R3 in the IP₃-bound state at 3.33 Å [1], is shown side-on with the
...
as Figure 1.4 shows: panel **a** draws the receptors of both
...
and IRBIT [19]. Figure 1.5 reduces that regulation to its logic:
...
![](figures/fig_1.5_gating_logic.png)

**Figure 1.5.** The figure draws what opens the channel and what
...
and Figure 1.6 sets that family out: panel **a** draws a blip, a
...
![](figures/fig_1.6_signal_hierarchy.png)

**Figure 1.6.** The calcium signal is built in tiers, and the
...
two. Figure 2.1 lays the clinical record out gene by gene with the
...
![](figures/fig_2.1_review_disease.png)

**Figure 2.1.** The curated disease-variant map records each
...
Figure 2.2 draws the three genes to one scale: panel
...
![](figures/fig_2.2_review_gene_architecture.png)

**Figure 2.2.** The three human genes are drawn to one
...
Figure 2.3 is that starting picture: panel **a** counts the
...
![](figures/fig_2.3_review_taxonomic_range.png)

**Figure 2.3.** The databases' own picture of the family's
...
into the definition of a gene family. Figure 2.4 shows the
...
![](figures/fig_2.4_review_family_separation.png)

**Figure 2.4.** The families are separated by evidence
...
human ITPR1 numbering. Figure 2.5 carries it in three panels:
...
Figure 2.6 sets those five windows beside the same
...
![](figures/fig_2.5_review_conservation.png)

**Figure 2.5.** Conservation is plotted per column across a
...
![](figures/fig_2.6_review_alignment_windows.png)

**Figure 2.6.** The two families agree at the pore and
...
The audit's last figure, Figure 2.7, is the regulatory layer as
...
![](figures/fig_2.7_review_regulation.png)

**Figure 2.7.** The curated regulator map shows what binds the
...
audited afterwards. Figure 3.1 shows that margin for every record in
...
![](figures/fig_3.1_census_margin.png)

**Figure 3.1.** The labelled-bait margin is measured across the
...
defining domain does not annotate. Figure 3.2 draws the space those
...
![](figures/fig_3.2_census_space.png)

**Figure 3.2.** The enumerated search space is drawn by signature
...
that satisfy neither positive test. Figure 3.3 breaks those three
...
not about range. Figure 3.4 then puts the two called families side
...
![](figures/fig_3.3_census_lineage.png)

**Figure 3.3.** The census is broken down by lineage with both
...
![](figures/fig_3.4_census_lengths.png)

**Figure 3.4.** The two families separate cleanly by length as
...
disagrees on 1. Figure 3.5 shows the two scores for every
...
**Figure 3.5.** Every record's two profile scores are
...
assumed. Figure 3.6 shows the division of labour: panel
...
**Figure 3.6.** Each record is called by one instrument, by
...
Figure 3.7 draws the three runs: panel **a** is the count of
...
**Figure 3.7.** Each seed's convergence curve is drawn with
...
themselves. Figure 3.8 shows the confound behind that list:
...
**Figure 3.8.** Copy number per proteome rises with
...
Read per paralogue, the evidence stacks as Figure 4.2 shows: nearly
...
Figure 4.1, one panel per paralogue, the same evidence fills almost
...
**Figure 4.2.** Each of the 927 genome-by-paralogue cells carries one
...
representatives. Figure 4.3a plots the fraction of cells found
in each bin of contig N50, and Figure 4.3b collapses that curve
to the two sides of the bar, 189 genomes above it and 120 below.

![](figures/fig_4.3_contiguity_confound.png)

**Figure 4.3.** Recovery rises with assembly contiguity, and
...
Counting loci per genome rather than per cell gives Figure 4.4,
...
**Figure 4.4.** The figure counts the family's loci per genome
...
172. Figure 4.5a plots the miss rate against contig N50 for both
series, Figure 4.5b scores the a-priori bar against the residual
miss rate at every candidate floor, and Figure 4.5c counts the
genomes each floor removes, which is the price quantified below.

![](figures/fig_4.5_s19_contiguity.png)

**Figure 4.5.** The false-negative rate falls with assembly
...
than against a model of it. The result is Figure 4.6: panel a puts each
...
**Figure 4.6.** Nineteen ablated panels are drawn as change from the full
...
so the labels are not a judgement call. Figure 4.8a traces the
sister-family share round by round for all seven runs, Figure 4.8b traces
the off-family share, which is the axis that actually moves, and
Figure 4.8c scores each rule's sensitivity and specificity against the
outcome.

![](figures/fig_4.8_s19_drift.png)

**Figure 4.8.** Each kill rule is scored as a classifier of a drift
...
135 clades swept do. Figure 5.1 lays that presence out phylum by
...
**Figure 5.1.** The family's presence across the eukaryotes is
...
rests on the two-profile margin, and Figure 5.2 plots each
...
**Figure 5.2.** The same two-profile separation holds
...
contamination artefact. Figure 5.3a gives the verdict on each
record, and Figure 5.3b plots each record's best identity
outside its own kingdom, which is the axis a contaminant would sit at the top
of.

![](figures/fig_5.3_plant_fungal_chase.png)

**Figure 5.3.** The chase of every plant and fungal record ends
...
Figure 5.4a traces each group's included set round by round to
convergence or to the kill, and Figure 5.4b traces the sister-family
and off-family shares over the same rounds.

![](figures/fig_5.4_jackhmmer_s20.png)

**Figure 5.4.** Each group's iterative search is traced to
...
pilot, 1 of 14 genomes was uncontrolled. Here, none. Figure 5.5
...
![](figures/fig_5.5_absence_at_genome.png)

**Figure 5.5.** Each absence claim is drawn with the number of
...
then two sponges at 8 and 6. Figure 5.6 plots that count by
group, with a line at three marking where the vertebrates sit.

![](figures/fig_5.6_nonvert_copy_number.png)

**Figure 5.6.** Copy number outside the vertebrates runs from
...
Figure 5.7 plots every recorded cluster's identity with the confirmed
...
**Figure 5.7.** Identity does not separate real genes from junk
...
The second threshold is the span a locus may occupy, and Figure 5.8
...
**Figure 5.8.** Locus span exceeds coding footprint by a factor that
...
fitted to the answer. Figure 6.1a rasterises residue
occupancy for all 134 representatives across the 11,777 input columns,
Figure 6.1b plots the mean occupancy along the same
axis, and Figure 6.1c marks the columns the trimmer
kept, which are the dense ones. Figure 6.2 then asks how much of the
kept set each representative covers, and names the single tip that covers
under half of it.

![](figures/fig_6.1_supp_representative_alignment.png)

**Figure 6.1.** The representative alignment is
...
![](figures/fig_6.2_msa_coverage.png)

**Figure 6.2.** Each representative's coverage of the trimmed
...
a heuristic with. Figure 6.3 shows that separation for every
pair at once, with the vertebrate paralogue blocks and the outgroup block
standing out from the rest of the matrix.

![](figures/fig_6.3_msa_identity_heatmap.png)

**Figure 6.3.** All-pairs identity across the representative
...
![](figures/fig_6.4_msa_group_identity.png)

**Figure 6.4.** Identity within and between groups is summarised
...
of Figure 6.4 does so while its right panel puts every group
...
Figure 6.5 carries the same alignment's per-column conservation
under the human ITPR1 domain track, so the elements Chapters 11 and 12 score
can be located on the columns the tree used.

![](figures/fig_6.5_msa_conservation.png)

**Figure 6.5.** Per-column conservation is drawn with human
...
Figure 6.7 lists the vertebrate tips the tree places in no
...
![](figures/fig_6.7_paralog_placement.png)

**Figure 6.7.** Each vertebrate tip's placement by the tree is
...
fails any constraint that names a free tip. Figure 6.8a shows what each
constrained topology costs in log-likelihood against the unconstrained tree,
and Figure 6.8b shows the test's verdict on each, with the rejection line
at p = 0.05.

![](figures/fig_6.8_sister_au.png)

**Figure 6.8.** The three sister hypotheses are compared under the
approximately unbiased test [55], with the log-likelihood difference and
...
Figure 6.9 places every internal node by its two support values at
once, and the 91 that sit past both thresholds are the ones a claim may rest
on.

![](figures/fig_6.9_support_profile.png)

**Figure 6.9.** Node support is drawn as a scatter of the two
...
individual pairs beating their own control. Figure 7.1 lays
...
![](figures/fig_7.1_synteny_pair_classes.png)

**Figure 7.1.** Each pair class stands beside its matched
...
Figure 7.2 makes the split, and what to read off it is the
...
![](figures/fig_7.2_synteny_clade_decay.png)

**Figure 7.2.** Neighbourhood similarity decays with
...
Both links are laid out in Figure 7.3a as shaded families on
the three human gene tracks, and Figure 7.3b sets each family's
...
![](figures/fig_7.3_synteny_paralogon.png)

**Figure 7.3.** The two surviving ohnologue links both run
...
toward the stricter setting. Figure 7.4a is the score distribution
the calibration reads, annotation-confirmed loci against random windows on
one axis, and Figure 7.4b is the sweep across the threshold. The
...
![](figures/fig_7.4_synteny_caller.png)

**Figure 7.4.** The neighbourhood caller is calibrated on loci
...
combinations are refused with their reason. Figure 7.5a lays out the
...
Figure 7.5b then scores the alternative roots, and the outgroup
rooting sits within a few events of the minimum-event one, so the root chosen
is not what produces the answer.

![](figures/fig_7.5_recon_matrix.png)

**Figure 7.5.** Each cell of the topology by variant matrix names
...
Figure 7.6 puts both placements on the dated species tree,
...
![](figures/fig_7.6_recon_dated_backbone.png)

**Figure 7.6.** The dated backbone is drawn on a linear time
...
Figure 7.7a shows those six distances as lines inside the
distribution of all vertebrate tips, and Figure 7.7b sets each
...
![](figures/fig_7.7_recon_cyclostome.png)

**Figure 7.7.** The six cyclostome loci are marked on the
...
sample, and none is corroborated. Figure 7.8a breaks the implied
...
species with no genome in scope. Figure 7.8b shows that every cell
of the matrix implies roughly 50 to 100 losses while the genome sweep
corroborates none.

![](figures/fig_7.8_recon_losses.png)

**Figure 7.8.** The genome ledger classifies each implied loss per
...
background rate, at 2.7 % against 2.6 %, p = 0.554.** Figure 7.9a
draws that replication for the three pairs against the random-window line,
and Figure 7.9b returns to the single human genome, counting each
...
![](figures/fig_7.9_s16_paralogon.png)

**Figure 7.9.** The two-round test is drawn with its null across
...
Figure 7.10a gives the mean copy number of each gene in the
...
at the teleost step. Figure 7.10b counts, per vertebrate class,
the genomes carrying more than one copy, and a second ITPR is essentially
confined to the ray-finned fish.

![](figures/fig_7.10_s16_copy_number.png)

**Figure 7.10.** Copy number is grouped by whole-genome-duplication
...
duplications would not. Figure 7.11a places each two-copy genome by how many
ancestral-block symbols each copy kept, and almost every genome keeps some on
both. Figure 7.11b counts the genomes meeting each condition against both
references, next to the line marking all 49.

![](figures/fig_7.11_s16_dcs.png)

**Figure 7.11.** Double-conserved synteny is shown by plotting the two
...
Figure 7.12a plots each anchor's assignment margin, which is positive
for every anchor, and Figure 7.12b recounts the multi-copy genomes of
each gene at seven coverage bars, where the counts stay flat across the
range.

![](figures/fig_7.12_s16_blocks.png)

**Figure 7.12.** Cross-anchor block identity is shown beside the
...
junctions read as a canonical splice pair.** Figure 8.1a draws
the complement of that rate for each paralogue and the ryanodine control,
Figure 8.1b shows that every gap bin from 6 bp upwards reads as
spliceable, and Figure 8.1c compares the boundaries with an
independent annotation pipeline; together they say the boundaries the rest of
this chapter counts are real.

![](figures/fig_8.1_junction_quality.png)

**Figure 8.1.** The aligner almost never places a boundary where
...
Figure 8.2a shows the exon counts holding at 57 to 58
against the ryanodine receptors' 104, Figure 8.2b shows the
genomic spans spreading 4.2-fold with ITPR3 the most compact, and
Figure 8.2c shows that the median counts do not move as the
bait-coverage bar is raised.

![](figures/fig_8.2_architecture_by_paralog.png)

**Figure 8.2.** The exon count is conserved while the
...
![](figures/fig_8.3_intron_positions.png)

**Figure 8.3.** The three paralogues share almost all of their
...
2.3-fold, significant in none of 188 genomes. Figure 8.3a shows
...
Figure 8.3b places every genome far above the chance diagonal for
...
Figure 8.4a gives the verdict for each split and
fragmentary locus, and Figure 8.4b shows where the 1,448
...
Figure 8.4c scores the detector against the independent
...
**Figure 8.4.** Most fragmentary annotations have at least
...
Figure 9.1a sets the candidates, the co-shattered paralogues and
the decoys side by side against the bar, and Figure 9.1b shows that
a gene spread over two contigs and one spread over twelve both land above it.

![](figures/fig_9.1_reconstruction.png)

**Figure 9.1.** The calibrated bar separates the candidate gene from
...
it acts. Figure 9.2a shows where the 432 regions sit, most of them on
a contig with no coding gene at all, and Figure 9.2b shows the caller
correct every time it calls, so what failed was reach and not accuracy.

![](figures/fig_9.2_synteny_reach.png)

**Figure 9.2.** Synteny could not answer because a trace region has
...
genome. Figure 9.3a lays out all 927 cells in a single strip
per paralogue, and Figure 9.3b turns them into a per-genome count
of gene-equivalents.

![](figures/fig_9.3_character_matrix.png)

**Figure 9.3.** Every genome by paralogue cell is drawn in the
...
Figure 9.4a and Figure 9.4b set the two correlations side by side,
and Figure 9.4c shows the comparison inside one genome that follows from
them.

![](figures/fig_9.4_integrity.png)

**Figure 9.4.** Lesion density is plotted against the two confounders
...
![](figures/fig_9.5_sensitivity_matrix.png)

**Figure 9.5.** Panel a gives the Dollo loss count in every cell
...
rests on. Figure 9.6a shows how far above the gap almost every
reassembled cell sits, and Figure 9.6b shows the absent count
...
family-level absence is not. Figure 9.5a is zero in all but its
bottom-right corner, while Figure 9.5b darkens as the protective rules
are switched off and the evidence ladder is tightened.

![](figures/fig_9.6_reconstruction_bar.png)

**Figure 9.6.** Each of the 43 reassembled cells is placed
...
argued. Figure 9.7a draws all twelve of those curves.

![](figures/fig_9.7_mk_profile.png)

**Figure 9.7.** Panel a profiles the likelihood along a rate grid for
...
here, namely setting the units a rate is quoted in. Figure 9.7b shows every
...
**Figure 9.8.** Panel a stratifies the identity-matched sign test by
...
six pairs cannot carry a test. Figure 9.8b draws that split, beside
the class strata of Figure 9.8a and the few fossil candidates of
Figure 9.8c. Two thirds of bird assemblies are below the
...
whole triplets only.
Figure 10.1 shows what survived: occupancy per codon over the
...
![](figures/fig_10.1_supp_codon_alignment.png)

**Figure 10.1.** The trimmed codon alignment carries every
...
Figure 10.2 draws the three rates on a logarithmic axis against the
...
![](figures/fig_10.2_omega_by_paralog.png)

**Figure 10.2.** Each paralogue's rate sits far below neutrality,
...
Figure 10.3 plots every within-paralogue pair for ITPR1, ITPR2 and
...
**Figure 10.3.** Pairwise rates within each paralogue reach
...
Figure 10.4 puts the two instruments side by side, the two-ratio
...
![](figures/fig_10.4_branch_contrast.png)

**Figure 10.4.** Two different statistics on two different models
...
Figure 10.5 shows all twelve restarts against their null likelihoods,
and the three that fall below the line are the failures a single run would
have printed as answers.

![](figures/fig_10.5_bs_restarts.png)

**Figure 10.5.** Every restart of the branch-site model is plotted
...
Figure 11.1 makes the shape of that shortfall visible: panel a
...
![](figures/fig_11.1_s11_afdb_coverage.png)

**Figure 11.1.** The prediction database's coverage of the family
...
Figure 11.2 lists every entry that resulted, with the claimed length and
...
![](figures/fig_11.2_s11_panel.png)

**Figure 11.2.** Every structure in the panel is drawn as two bars: the
...
is there because a divergence from a background frequency table [93] scores
...
written.
Figure 12.1 lays the two definitions out residue by residue,
...
![](figures/fig_12.1_supp_labelled_positions.png)

**Figure 12.1.** The two modules are drawn at residue
...
Figure 12.2 makes the reversal visible: panel a marks where the two
...
![](figures/fig_12.2_s22_modules.png)

**Figure 12.2.** The core is tested against the pore under both pore
...
particular residues.
Figure 12.3 plots every residue of the pocket against its distance from
...
![](figures/fig_12.3_s22_shells.png)

**Figure 12.3.** Constraint is plotted against distance from the ligand
...
pocket.
Figure 12.4 carries the rate axis: panel a compares the per-site rates of
...
![](figures/fig_12.4_s22_omega.png)

**Figure 12.4.** Per-site substitution rates are compared by module in
...
p = 0.87.**
Figure 12.5 lays the whole argument out in three panels: panel a is the
...
![](figures/fig_12.5_s22_lineage.png)

**Figure 12.5.** The lineage strata are drawn with the power curve
...
single annotated model delivers, and Figure 13.1a shows where it
...
![](figures/fig_13.1_s18_calibration.png)

**Figure 13.1.** The completeness bar sits inside the
...
submitter-deposited ones.** Figure 13.2a stacks the five locus
...
![](figures/fig_13.2_s18_by_source.png)

**Figure 13.2.** Locus state depends on the annotation source, and
...
6.7 % and 5.5 %. Figure 13.3a stacks the five states for
...
**Figure 13.3.** The family fails at the rate its sister
...
Figure 13.4a draws that split for the gene symbol and for the
...
**Figure 13.4.** The protein records are mostly correct where
...
single occupied bar of Figure 13.4b.
...
Figure 13.5a is the distribution the cases come from, annotation
...
![](figures/fig_13.5_annotation_loss.png)

**Figure 13.5.** Annotation loss is a tail rather than a norm: of
...
Figure 13.6 draws the two loci as they sit on their contigs, Case A
...
![](figures/fig_13.6_exon_tracks.png)

**Figure 13.6.** The two loci are drawn at true genomic width, with
...
Figure 13.7 then asks the annotation's own proteins where they
...
**Figure 13.7.** Tiling the annotated proteins back onto the
...
Three of the five checks can be drawn, and Figure 13.8 draws them
...
**Figure 13.8.** Three of the five checks on each case are drawn
...
own reversed decoys at zero. Figure 13.9 shows the pooled read count
...
![](figures/fig_13.9_s12_detection.png)

**Figure 13.9.** Every recovered locus collects reads far above its
...
Figure 13.10 lays this out junction by junction for each lost gene,
so a reader can find every junction that no read crossed rather than take the
pooled percentage on trust.

![](figures/fig_13.10_s12_junctions.png)

**Figure 13.10.** Reads cross the junctions the annotation does not
...
Figure 13.11 puts the two instruments on one axis. The loci with no annotated
...
Figure 13.12 compares the two instruments for the same twelve loci:
...
**Figure 13.12.** The deposit cross-check cannot answer where the
...
is about the search and not about the family. Figure 14.1 splits each
...
**Figure 14.1.** A domain enumeration added almost everything a
...
31. UniProt Consortium. UniProt: the Universal Protein Knowledgebase in 2023. *Nucleic Acids Res* **2023**. PMID 36408920. doi:10.1093/nar/gkac1052.
...
100. Leinonen R, Sugawara H, Shumway M, International Nucleotide Sequence Database Collaboration. The sequence read archive. *Nucleic Acids Res* **2011**. PMID 21062823. doi:10.1093/nar/gkq1019.
