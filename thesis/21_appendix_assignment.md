...
**The methods results sit entirely in Chapter 4**, so the measurement of
what the search was worth stays with the search it measures. S26's own
starting
...
**The thesis's reason is that the number is the instrument's error bar.** The
false-negative rate is a property of the search,
...
**The series' reason is that a paper carries its own controls.** A retention
claim without its own false-negative rate
...
**A reader holding both documents should take one thing from it.** The thesis
and
...
`thesis/guard_check.tsv`. **All 30 fire with the message they are supposed
...
| a legend opening on a fragment (R1) | a legend opener rewritten as a noun phrase | `legend <slug> opens without a finite verb (R1)` |
| a legend sentence that is a fragment (R2) | a later legend sentence rewritten as a noun phrase | `legend <slug> has a sentence with no finite verb (R2)` |
| a lead-in that is a fragment (R1b) | a bold paragraph lead-in rewritten as a noun phrase | `bold lead-in has no finite verb (R1b)` |
| an undescribed figure (R3) | every body reference to a placed figure deleted | `figure <slug> is placed but no sentence of chapter N refers to it (R3)` |
| a legend set as body text (R4) | a legend paragraph left outside the legend typography | `legend not wrapped in its own typography` |
| overlapping labels (R5) | two text artists drawn on top of each other | `text '<a>' overlaps text '<b>'` |
| an annotation never drawn (R5) | an annotation anchored outside its axes | `text '<a>' is not drawn: its anchor lies outside the axes` |
| a clipped label (R5) | a label cut by its axes' clip box | `<stem>: 1 figure-text problem(s)`, naming the clipped text |
| a panel title that is a fragment (R2) | a panel title with no finite verb | `title has no finite verb: '<title>'` |
| a crowded panel title (R5) | a title set within 3 pt of the next panel's letter and title | `title '<a>' has less than 3 pt clear of title '<b>'` |
| a title running into the next panel (R5) | a title drawn across a taller neighbouring panel | `title '<a>' runs into another panel's area` |
| a legend over its data (R5) | a legend placed on top of the line it keys | `legend '<a>' is drawn over N point(s) of the data it keys` |

Sixteen of these are the ones this thesis added over the manuscript's build:
the padded-claim guard, the three prose guards that follow the dropped-glyph
row, and the twelve at the foot of the table, which the revision pass added.
The last three were written after the first page-by-page read of the typeset
document found panel titles cut off by the panel beside them and legends
printed over their own data, none of which a text-against-text check can see. The
