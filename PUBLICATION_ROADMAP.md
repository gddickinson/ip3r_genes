...
> **Next session: S20b then S23.** S20b is two jackhmmer runs left iterating (protista, invertebrates) — collect them with `s20_jackhmmer.py --all --parse-only` and re-render. Then S23 takes S20a's absences to assembly level: Streptophyta 0/384, Dikarya 0/1,353, Apicomplexa 0/60, the patchy fungal losses, and the *Cymbomonas* copy-number question. Read `results/s20_sweep/report.md` first.
...
| S20b | **The remaining per-group convergence runs** — jackhmmer to convergence over `protista_other` and `metazoa_nonvert` under D10 | S20a | in_progress | Split out because the per-round cost is the **alignment, not the search**, and it does not fall with more cores: a round over a group with thousands of included targets costs ~45–65 min at any thread count this machine can give it. **`protista_other` is done: 10 rounds, killed by K3** — the third distinct outcome from the same criterion and the sharpest **D10b** case yet. It grew 721 → 26,148 targets but **never by more than 3.18× in one round, so K2 never fired**, and its sister share *fell* (0.28 % → 0.16 %, `sister_rise` negative in 6 of 10 rounds), so K1 moved the wrong way. Only the ceiling caught it. It accreted **11,344 Apicomplexa proteins** — a clade this task's own sweep called the family in 0/60 proteomes. D10 says no completeness claim may rest on a K3 run, but `s3_kill` still reports its pre-ceiling rounds as `accepted` (correct for K1/K2); the composition rows now carry the verdict and a `supports_completeness` flag and the report presents a disowned run separately. **Family saturation is read off every run regardless**: the family count settles at round 2 (viridiplantae, 54), 3 (protista, 729 — exactly the single pass's number, after which the model grew 1,995 → 26,148 adding none) and 4 (fungi, 35), so iteration finds no receptor the single pass missed in any group. **NEXT: `metazoa_nonvert` only** — launched in this session, at round 7 of 10 and converging (new targets 1,952 → 407 → 188 → 50 → 69 → 73), writes atomically so an interrupted run leaves a `.log.part` the driver will not parse. Collect with `python3 scripts/s20_jackhmmer.py --all --parse-only`, then re-render `s20_figures.py --only jackhmmer_s20` and `s20_report.py`. |
| S23 | **Targeted invertebrate + protist + plant/fungal genome sweep** (triggered by S20): the negative claims at genome level, and copy number outside vertebrates | S20a | pending | S20a sharpens the target list: Streptophyta (0/384) and Dikarya (0/1,353) are the headline absences to take to assembly level, plus **Apicomplexa 0/60**, the patchy fungal losses (Glomeromycota, Mortierellomycota, Kickxellomycota, Microsporidia) and the *Cymbomonas* copy-number question. |
