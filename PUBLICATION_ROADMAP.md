...
| S28 | **The revision pass** — read the assembled thesis as a reader: legends and in-figure labels written as statements rather than noun-phrase fragments, every figure described in its chapter's text, legends typographically distinct in the typeset document, and every layout defect on the page found and fixed | S27 | pending | **Measured before the task starts, to be re-measured on entry:** 31 of 104 legends open with a first sentence carrying no finite verb; 17 of 37 in-figure panel titles are fragments, concentrated in `s0_figs_genomics.py`, `s3_figures.py`, `s0_figs_sequence.py` and `s0_figs_structure.py`; Figure 1.1 is cited but never described in the body; a legend is set in the body font at body size in the PDF; the markdown sources are clean on spacing, so the spacing defects are on the rendered page and inside the figures. **This is the third report of the same fault** — D76 rewrote the headings, D78 recorded that the first pass had only fixed what somebody noticed and added checks for headings and bold lead-ins, and neither touched figure legends or the text drawn inside figures. So the brief's six rules each have to end the task as a mechanical check: a finite verb in every legend's opening sentence and in every in-figure label, a body-text description for every figure, a distinct legend typography, and an overlap-and-clipping detector inside `figstyle.save()` so a figure with colliding labels cannot be written at all. Brief: `docs/session_briefs.md`. |
