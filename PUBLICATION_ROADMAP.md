...
> **Next session: S11 — Structures.** S10 is complete:
> `results/annotation_bugs/` holds the ranking every recovered locus is
> scored in, and the two annotation failures validated down to the exon.
> Read `results/annotation_bugs/report.md` first. Three things S11
> inherits. **The census is now audited from the genome side**: the
> annotation delivers the whole gene at 375 of 382 loci where it
> demonstrably could, so the family's database record is good where it
> exists and the gaps are concentrated in a handful of genomes. **A locus
> with no protein record cannot have an AlphaFold model**, so S11's AFDB
> coverage denominator has to be census *records*, not census loci —
> `database_records.tsv` shows two swept species with three complete genes
> each and zero ITPR protein records between them, and there are more.
> **And the negative-control habit transfers**: S10's genomic control
> caught a real flaw in its own counting rule on first run — an 8 nt anchor
> admits an alignment that has run a dozen bases into the intron — so
> S11's TM-align panel needs its two negative controls (an unrelated
> channel of similar size, a large non-channel) built in from the start
> rather than added if the numbers look odd.
