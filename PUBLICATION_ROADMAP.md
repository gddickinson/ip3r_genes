...
| S20b | **The two remaining per-group convergence runs** — jackhmmer to convergence over `protista_other` and `metazoa_nonvert` under D10 | S20a | in_progress | Split out because the per-round cost is the **alignment, not the search**, and it does not fall with more cores: a round over a group with thousands of included targets costs ~45–65 min at any thread count this machine can give it, so these two need ~5 h and ~8 h that the two groups carrying the task's negative claims did not. Seeds already derived by rule and committed in `jackhmmer_seeds_s20.tsv` (*Capsaspora owczarzaki* 2,951.7 bits for the protists, *Parnassius mnemosyne* 4,762.4 bits for the invertebrates); both runs were launched in this session and write atomically, so an interrupted one leaves a `.log.part` the driver will not parse. **NEXT: `python3 scripts/s20_jackhmmer.py --all --parse-only`** once `<data_root>/hmmer/s20/jackhmmer_s20_{protista_other,metazoa_nonvert}.log` exist, then re-render `s20_figures.py` and `s20_report.py`. Expectation to test, not to assume: fungi was killed by **K2** at round 3 with the growth entirely off-family and K1 flat at 0.000, and these two groups are larger, so the same failure is likely — but D10's verdict is what the run says, not what the previous run said. |
