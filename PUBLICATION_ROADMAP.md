...
| S5b | **The full 309-genome sweep** → per-genome ledger (found / lost / assembly-gap) + novel gene models → census v4 | S5a | in_progress | **Established so far** (sweep still running). *Instrument hardening:* the **chunked miniprot path is validated** — `s5_test_chunked.py` proves the two paths return identical loci, baits, coverage and cell statuses (worst boundary difference 9 bp), and it showed the mRNA-ID collision is **real** (167 rows, 60 distinct IDs over 6 chunks), so the guard in `parse_miniprot_gff` is load-bearing. Measuring peak RSS at **6.3–8.0 GB per Gbp** put the ported 4 Gbp chunk at ~25 GB on a 34 GB machine, so `CHUNK_BP` is now 2.5 Gbp. **miniprot output is now written atomically** — it was written straight to `miniprot.gff`, which the driver reuses if non-empty, so an interrupted run left a truncated file the next run accepted as complete and which is undetectable afterwards (a GFF cut at a line boundary parses perfectly). Audited: 74 completed GFFs all end on a complete record and **0 genomes reused a pre-restart GFF**, so nothing was corrupted. `s5_test_resume.py` proves a failed run leaves nothing reusable and that a re-run reproduces its summary **including through miniprot** — D24's question asked of a second tool, and miniprot passes where MAFFT did not. *Results:* the **fragment-level margin calibration S5a deferred is done** — rescue regions overlapping a gene the assembly names for a paralog agree with the attribution 11/11 at margins 0.227–0.370; 0 fall below the 0.22 in force, 7 would have fallen below the inherited 0.333. **The budget was wrong**: the sweep is download-bound (~5.5 MB/s aggregate, not scaling with shard count), ~8 h transfer against ~2.5 h compute, where S5a projected 3.4 h from compute alone; miniprot itself is *faster* than budgeted (15 s/Gbp, not 36). **Census v4 records how a database holds a locus**, not only whether. 
> 
> **NEXT (resume point).** The sweep is running detached: six shards (`s5_run_sweep.py --shard i/6 --threads 3`, PPID 1) plus a supervisor (`<data_root>/genome_sweep/_overnight.sh`) that logs to `_progress.log` every 10 min and then sweeps the two giants (*Lissotriton helveticus* GCF_964261635.1, *Protopterus annectens* GCF_019279795.1) on the chunked path once the shards drain. **To resume: check `_progress.log`; if `FINAL:` is present the sweep phase is done.** Then run, in order: `s5_ledger.py`, `s5_calibrate_margin.py`, `s5_census_v4.py`, `s5_budget.py`, `s5_figures.py`, `s5_report.py`. Re-run `s5_run_sweep.py --retry-failed` first if any `.sweep.failed` markers exist. **If the current run is interrupted, delete `miniprot.gff` for every genome lacking `.sweep.done` before resuming**: the running shards loaded the pre-fix module and still write that file non-atomically, so a kill mid-write can leave a truncated GFF the next run would accept. **Decide the attribution margin last**: it is 0.22 from S5a's complete loci, the fragment-level evidence now measures it directly (`margin_calibration.json`), and a change to it needs only the rescue attribution re-derived, not the sweep re-run. **The budget was wrong and is corrected**: the sweep is download-bound (~5.5 MB/s aggregate, not scaling with shard count), ~8 h of transfer against ~2.5 h of compute; S5a's 3.4 h assumed compute alone. | Budget measured, not estimated: **303 genomes, 547 Gbp, 6.7 h compute (miniprot 36 s/Gbp, 8 threads), 3.4 h wall at 2 shards, 164 GB download**; 547 GB of FASTA fits in 1,427 GB free, so keep it for S8/S10. Two giants (*Protopterus* 40 Gbp, *Lissotriton* 23 Gbp) need the chunked path and are outside the driver's 12 GB default cut. Re-calibrate `ATTRIBUTION_REL_MARGIN` on rescue regions of known identity, and test the span-bias result at scale. |
