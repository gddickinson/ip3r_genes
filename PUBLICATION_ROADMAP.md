...
| S9b | **The model-based selection tests** — codeml one-ratio, pairwise, two-ratio, branch-site model A on each paralog stem, M1a/M2a + M7/M8 site models, and HyPhy RELAX, with every LRT BH-corrected | S9a | in_progress | **First results, on the paralogs whose jobs have landed:** ITPR1 ω = **0.0238**, ITPR2 ω = **0.0430** (curated-CDS subsets 0.0212 / 0.0318, so the genome gene models are not driving them). **Synonymous saturation is worse than the prior expected and reaches inside a paralog, not only between them** — 94 % of within-ITPR1 and 85 % of within-ITPR2 pairs exceed dS = 1.5, dN plateaus near 0.1 while dS runs past 50, so the pairwise matrix is a diagnostic here and every reported ω comes from a tree-based model. Branch-site model A is restarted from four initial ω **by construction** (**D37**), not repaired afterwards. **NEXT:** the codeml suite is running and fully resumable — `python scripts/s9_codeml.py --workers 5` (40 jobs; site models and whole-tree branch models are the long poles), then `s9_relax.py`, `s9_tables.py`, `s9_report.py`, `s9_figures.py`. |
