...
> **Next session: S23c.** The 194-genome sweep is *running* — resume it with
> `python3 scripts/s23_run_sweep.py --threads 8 --smallest-first` (it skips
> completed genomes on their `.sweep.done` markers), then run, in order:
> `s23_calibrate_loci.py` → a `--redo` reclassify pass to apply the measured
> floor → `s23_ledger.py` → `s23_figures.py` → `s23_census_v6.py` →
> `s23_report.py`. Read `results/s23_scope/report.md` first: it carries the
> partial sweep and the three thresholds S23b re-measured. **Do not skip the
> reclassify pass** — until `locus_calibration.json` exists the sweep is
> running on S5b's inherited 0.40, which `s23_calibration.call_min_identity()`
> reports as an inheritance rather than a measurement.
...
| S23c | **The full 194-genome sweep** → per-genome copy-number ledger + the plant/fungal/apicomplexan absence claims at assembly level → census v6 | S23b | in_progress | **127 of 194 swept as of 2026-09-06, 0 failures, running.** NEXT: the sweep resumes with `python3 scripts/s23_run_sweep.py --threads 8 --smallest-first` (per-genome `.sweep.done` markers; ~8 h of miniprot left, 92.5 Gbp). Then, in order: `s23_calibrate_loci.py` (writes the measured `call_min_identity`), a `--redo` reclassify pass to apply it (miniprot GFFs are cached, ~40 min), `s23_ledger.py`, `s23_figures.py`, `s23_census_v6.py`, `s23_report.py`. **At 127 genomes, all 5 downstream modules run clean and the partial result is already substantial**: 34 of 35 absence clades have at least one controlled genome and **every one holds at assembly level** — Ascomycota 0/31, Basidiomycota 0/17, Apicomplexa 0/3, Streptophyta 0/6 (one trace). Controls: 87 cross-kingdom, 23 by-target, 5 own-clade, 1 RyR-only, 9 `no_control_bait` (protist clades carrying no absence claim), **2 `uncontrolled`** — *Intoshia linei* and *Allopauropus danicus*, both reduced metazoan genomes, correctly excluded from every absence claim. Copy number 0/1/2/4 = 104/17/5/1; the 4 is the annelid *Dimorphilus gyrociliatus*. **The one fungal positive so far is *Basidiobolus meristosporus* with 2 complete models** — Zoopagomycota, not Dikarya, so the Dikarya absence is untouched by it. |
| S6 | Alignment upgrade: MAFFT L-INS-i + trimAl on census representatives per clade × kingdom, RyR outgroup included | S2, S3, S5b, S20a | pending | |
| S7 | ML phylogeny: IQ-TREE 2 + ModelFinder + 1000 UFBoot, rooted on RyR; resolve which two of ITPR1/2/3 are sisters (AU test on the three rooted topologies) | S6, S23c | pending | |
...
| S14a | **Manuscript assembly** — draft, figures, methods, deposit manifest, reviewer self-audit | S3, S5b, S7, S8, S9, S10, S11, S20, S23c, S15–S19 | pending | |
