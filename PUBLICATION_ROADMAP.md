...
> **Next session: S13 — Reconciliation & dating.** S12 is complete:
> `results/expression/` holds 67 streamed RNA-seq runs and the answer to
> the question S10 handed forward. Read `results/expression/report.md`
> first. Three things S13 inherits. **The annotation failures are settled
> as biology, not as artefacts** — all 7 are transcribed, spliced genes, so
> any loss S13 reconciles must not be built from a database absence that
> S10 and S12 have already shown to be an annotation absence; the loss
> audit S13 owes the ledger has to distinguish those two. **A gene can be
> annotated and still reach no protein database**: two *Nibea albiflora*
> loci are filed as pseudogenes and one is unannotated, so "present in the
> census" and "present in the proteome" differ by more than S3 measured,
> and a reconciliation counting presence from protein records would score
> that species as three losses. **And the habit of measuring an inherited
> constant rather than carrying it over paid three times here** — a
> validation floor, a decoy floor and a cross-mapping assumption all
> changed on measurement, and two of them changed the reported result. D15
> asks S13 to hand-curate the species tree as an input with a literature
> age on every calibrated node; that is the same discipline applied to a
> number nothing in this project can derive.
