...
> **Next session: nothing autonomous remains.** Every ledger row is complete
> except **S14b (deposit and release), which is human-gated**: a Zenodo DOI,
> the repository made public (D2), re-checking that each cited DOI and PMID
> resolves, and preprint uploads. S26 changed what S14b releases. There are
> now three reports of one body of work: the single manuscript
> (`manuscript/`), the thesis (`thesis/`) and **six papers (`papers/`)**, each
> with its own deposit manifest. Which of these goes to a journal is the
> human's decision, and `papers/README.md` is the page to decide it from.
>
> **Before any release, read the open Emergent rows dated 2026-09-23.** Two
> are corrections S26 made to text that had been wrong for several editions:
> the deep constraint layer ranks third of four classifiers, not second, and
> the 1,232 "demonstrated genes" include the 309 ryanodine receptor control
> genes (744 of 923 for the IP₃ receptor genes alone). Both are now fixed in
> the manuscript, the thesis, the S17 report and the README. `FINDINGS.md`
...
| S10 | Annotation-bug molecular validation: assembly-version audit, RNA-seq spanning evidence, miniprot gene models, for the two worst cases S5b/S18 surface | S5b | completed 2026-09-08 | **The annotation gets the gene right at 375 of 382 loci (98.2 %) where it demonstrably could** — 880 recovered loci, 382 passing E1–E5, median annotation loss 0, 360 at exactly 0. The 7 failures are in **3 genomes**: 3 omissions, 2 truncations, 2 fragmentations. **E5 is the rule that does the work** — *does this annotation build genes this long anywhere else in the same genome?* — and it removes exactly 6 loci in 2 genomes (*Cirrhinus mrigala*, longest annotated gene anywhere 42 kb; *Saguinus oedipus*, 138 kb against a 726 kb locus) and nothing else, so a genome-wide length ceiling cannot be reported as a bug at this gene. Two cases validated to the exon, one per mode. **Case A — *Nibea albiflora* ITPR2 (omission):** 56 coding exons, **all 56 with no annotated gene model**, in a chromosome assembly with 84× headroom whose annotation gets ITPR1 and ITPR3 right; the gene splices into 2,673 codons with **0 internal stops** (14.2 expected under neutral drift), 55/55 introns spliceable, 52/55 exon boundaries shared by a majority of 35 independently annotated genomes, and it sits between **SSPN and BHLHE41** — ITPR2's two most conserved flanks across 197 and 183 of 215 swept vertebrates. **The same genome files every complete family gene as a pseudogene** (8 of 14 family-named models, 31.6 % of the whole gene set is pseudogene) and **names its ITPR3 locus ITPR2** — the annotation's own translated protein matches the ITPR3 locus at 89.3 %, which adjudicates one of the three naming conflicts S5b deliberately left open. Net effect: *N. albiflora* has **0 ITPR protein records** in any database. **Case B — *Dissostichus eleginoides* ITPR3 (fragmentation):** one 68 kb gene called as **three** protein-coding models tiling residues 1–67, 52–467 and 468–1593 with the 3′ 41 % unmodelled; 59/59 introns canonical, 59/59 boundaries shared by a majority of 40 genomes, 0 internal stops. Both species: **0 ITPR protein records, 3 complete ITPR genes in the DNA.** The junction transcript search is a real negative with its denominator stated (43 and 10 transcript records exist; RNA-seq does, and goes to S12), and its **own negative control caught a flaw in the counting rule** — an 8 nt anchor admits an alignment run a dozen bases into the intron, so 6 of 55 junctions 'spanned' genomic DNA until a probe-coverage requirement was added. 47 negative controls run before any table is written. → `results/annotation_bugs/report.md` |
| S11 | Structures: AFDB coverage, TM-align vs the cryo-EM IP3R and RyR references, Foldseek AFDB-wide sweep | S2, S6 | completed 2026-09-08 | **AlphaFold DB does not hold this family.** 20.9 % of the census's 8,319 UniProt-shaped ITPR records have a usable (≥ 95 % covered) model — but of the **5,861 records at or above the family's own 2,000 aa floor, 13 do (0.2 %)**. Median modelled record 392 aa, median unmodelled 2,674 aa: coverage is a function of length, not taxonomy, and it is concentrated on the fragments. Of S6's 134 representatives — the set every alignment, tree and selection result stands on — **9** have a model. **AFDB answers a canonical accession with an isoform**: `Q14643-4` (2,695 of 2,758 aa) and `Q14571-2` — **181 residues of a 2,701-residue ITPR2** — so `afdb_probe` ranks every record in the response and the 181-aa model is rejected by the panel's minimum-chain rule, not used. **References resolved by RCSB query, family assigned by this project's census, never by title**: 237 entries / 414 entities enumerated on the family Pfams (the union, because RCSB's annotation of 6DQN carries no PF08709 — a PF08709-only query misses the project's own IP3R reference); 117 eligible; primary references 7LHF (ITPR1, 2.96 Å), 9YKK (ITPR2, 2.95 Å), 8TKG (ITPR3, 2.50 Å), 9NMO (RYR1, 2.40 Å), 7U9X (RYR2, 2.58 Å); **no full-length RYR3 cryo-EM entry exists**; a 6-state ITPR3 panel; 3 negative controls derived from S1's committed decoy panel (CACNA1A, Tln1, DYNC1H1), the MIR-sharer class unfilled. **D14 confirmed by a sixth instrument that reads only coordinates: 20/20 (100 %) of the callable census-named structures agree, and 0/3 negative controls receives a family call.** That last needed a fix the controls found: gated on the margin alone all three controls are called ITPR (they beat their runner-up by ~30 % while scoring 0.17–0.27), so a family call now requires the winner to clear TM-align's own 0.50 same-fold bar first (`CALL_FLOOR`). Calibration: same protein different state 0.78, IP3R×IP3R 0.43, IP3R×RyR 0.39 (0.64 the other way — the two cross the fold bar **in one direction only**, which is a size statement as much as a fold one), controls 0.19 with 0/81 pairs reaching 0.50. **Per-domain pLDDT: the IP3-binding core is the best-modelled domain (83.9) and the pore the worst (71.0)**, against 69.5 outside annotated domains — so S17/S22 can stand on predicted structures where they are about the ligand core. **Foldseek over AFDB Swiss-Prot returns the family and nothing else**: 36 hits confirm census ITPR records, 545 fall below the fold bar, and all 8 distinct above-bar non-census hits are SDF2/SDF2L1 across five kingdoms carrying **PF02815 and nothing else** — the MIR class the PDB could not control for. Zero novel structural leads. AFDB holds no RyR model at all, so a sister-family verdict there is unreachable by construction. 6 non-vertebrate models are **declined**, not contradicted: each had the arithmetic headroom (ceiling 0.50–0.63) and reached 0.56–0.74 of it. → `results/structures/report.md`, 14 tables, 4 figures |
| S12 | Expression evidence (SRA junction-spanning reads + atlases) for whichever paralog or lineage the census leaves in doubt | S5b | completed 2026-09-08 | **All 7 of S10's annotation failures are real, transcribed, spliced genes — 298 of 314 (94.9 %) of the junctions no annotated model spans are crossed by reads.** Scope derived from S10's committed ranking rather than chosen (4 rules): every eligible locus above 0.5 annotation loss, plus every other family locus in the same genome as an internal control. That is 7 failures across 3 species covering all three failure modes, with 5 controls. **67 public RNA-seq runs, 536,000,000 reads, 16 studies, 11 tissues**, streamed `fastq-dump \| hisat2` against a per-species reference of spliced genomic CDS. Each failed locus is detected in 13–31 of its runs and in 6–9 tissues; 15,263 reads read through their splice junctions. **At full depth the junctions the annotation does not model are crossed at the same rate as the ones it does (94.9 % vs 95.2 %)** — the apparent gap at partial depth was 3' bias, and the report prints the median junction position of both classes rather than leaning on the ratio. **Read coverage falls outside the annotation in the proportion S10 measured from the coding footprint** (7/7 within 0.25; four loci at 1.00 vs 1.00) — the same quantity from aligned reads instead of a GFF. **New: *Nibea albiflora* has no IP3 receptor in any protein database by any route** — ITPR2 is unannotated and ITPR1/ITPR3 are annotated *as pseudogenes* (0.98 / 0.97 of the coding footprint), and all three are transcribed and spliced in 31–32 of 32 runs. **The deposit cross-check was re-asked and is still `underpowered`** — now with a species that should have answered it: *D. mawsoni*'s 37,166 mRNA records return 0 spanning hits, and 0 for the annotated RyR control too; median deposit length 547 nt against an 8 kb transcript. The genomic negative control fires (1,791 hits, 0 spans), so the criterion discriminates. **Three inherited assumptions were measured rather than carried over.** (1) The translation-identity floor for validating a reference is the wrong instrument — a correct reference scores 1.00 with no frameshifts and 0.92 with nine; replaced by colinear block placement, which has no tuned threshold (reversed order 1.7 %, wrong strand 0 %, committed per locus). (2) The PIEZO decoy floor of zero **does not transfer**: 42 reads in 2 of 469 run x locus comparisons, both on one decoy, confined to ~64 bp — bounded and localised, and no detection call moves. (3) The closed-set risk is **measured, not argued**: every reference tiled with synthetic reads gives 0 of 12,500 cross-mapped. Housekeeping anchors are unequal for a measured reason — EEF1A1 has 5–6 genomic copies, GAPDH 3, RPL13A 1–3, and copy number orders them exactly. 11 negative controls, mutation-tested on 5 deliberate rule breakages, all caught. → `results/expression/report.md` |
| S13 | Gene-tree/species-tree reconciliation — date the duplications that made ITPR1/2/3 | S7 | completed 2026-09-08 | **Both duplications are placed, and they are not on the same branch.** The split separating **ITPR1 from ITPR2+ITPR3 is on the vertebrate stem** — older than crown Vertebrata (published estimates 480–615 Ma), with no upper bound this tree can set — and the split separating **ITPR2 from ITPR3 is on the gnathostome stem**, bracketed **462–563 Ma**. → `results/reconciliation/` (report.md, 11 TSV/Newick artefacts, 4 figures, stats json). **Species tree is an input, not a result (D15)**: 31 species, 29 named internal nodes, each with a literature age, the spread of published estimates, a **stem age** and its source, validated by `s13_species_tree.py --check`. The stem-age column is new against the PIEZO port and load-bearing — the sampled tree omits unsequenced lineages, so a duplication on Boreoeutheria would otherwise be bracketed [96, 319] Ma instead of [96, 99]. **Matrix design, 5 topologies × 3 variants = 12 reconciliations + 3 refused**: the topology never moves the answer (all five give 14 dup / 53 loss with the cyclostome loci and 12 / 51 without, including the two AU-rejected sister arrangements and the `--bnni` guard), so what decides the placement is taxon sampling, not sister order. **The whole difference is six cyclostome tips**, and specifically one hagfish+lamprey pair (100/100) nested sister to the ITPR2+ITPR3 clade: a lineage carrying both cyclostome and gnathostome copies forces the older duplication above the cyclostome–gnathostome split, while that same pair sitting *outside* ITPR2 and ITPR3 is what keeps the younger one below it. **Three checks say how much weight those tips take.** (1) The placement survives collapsing every node below S7's own support bar — the ITPR1 split sits on a 17.4/54 node, and collapsing it merges two vertebrate-stem duplications into one carried at 100/100, so the answer costs resolution, not placement. (2) Re-rooting the subtree at all 112 edges: minimum-event rooting picks a *different* edge (64 events against the outgroup rooting's 67) and **agrees** on the placement. (3) **The long-branch objection does not apply** — the six cyclostome tips are 0.96–1.04× the median root-to-tip distance, ranking 18–55 of 57. **S8's handed-forward question is answered and its own evidence stays underpowered**: all six loci join exactly and offline to an S8 locus (3 by coordinates, 3 by census-v6 `LOC` symbol), and of the tree's three orthology pairs S8's flank caller agrees on 1, disagrees on 1 and is uninformative on 1, with every one of its 4 calls inside its own null. **Loss audit is again the methodological result**: of 51–53 implied losses, **0 survive contact with the genomes** — 26 sampling artefacts, 4 `paralog_unassignable`, 17 out of scope. That last verdict had to be added: without it the only four "corroborated losses" were ITPR2/ITPR3 in the two cyclostomes, whose genomes carry **three ITPR loci apiece**, all filed in the ITPR1 cell because S5 has no cyclostome-labelled bait — a bait-panel limit reading as biology. **Two coded bugs caught by the self-tests, both silent**: the classic binary LCA duplication rule reads the collapsed four-way root as a *speciation* (fixed with the non-binary rule, Vernot et al. 2008), and `collapse_unsupported` dissolved all three constrained topologies into one 134-tip polytomy because a constrained IQ-TREE search writes no support values. 14 self-test groups, mutation-tested on three deliberate breakages. **GeneRax not run**: the PIEZO project established that bioconda 2.1.3 segfaults on both macOS builds; the matrix stands in its place and is reported cell by cell |
...
| S25 | **The thesis** — assemble the work as a thesis-length document: chapters not sections, the reasoning behind every instrument written out, and a bibliography audited on entry | S14c, S24 | completed 2026-09-09 | **73,494 words, 16 chapters, 5 appendices, 104 figures, 116 references, 194 pages.** Built by `python scripts/s25_assemble.py` — ten stages, exit 0. **Chapter 16 describes how the project was carried out with Claude Code** — the one-task session protocol, the literature audit, the download scope decided by rule, the unattended analysis runs, and the generation of every table, figure and document — with its numbers measured from the repository on every build (`thesis/production_stats.tsv`) rather than recalled. **The chapter grouping is committed before the prose** (`thesis/chapter_rules.md` + `chapter_assignment.tsv`): seven rules, four enforced by the build, 37 results directories assigned across 13 chapters with one excluded and a reason. **The reference audit caught 9 of 76** — nine identifiers written from memory resolved to entirely different papers (a duplication-inference algorithm's returned MrBayes, a reconciliation method's a real-time-PCR paper, an ancestral-genome reconstruction's a paper on structured RNAs), each of which would have entered a bibliography looking normal. **Nothing bibliographic is typed**: a new reference is declared by identifier plus a phrase its title must carry, resolved live and written from the fetched record. **234 load-bearing numbers verified, and the ledger is closed in both directions** — 151 carried from the manuscript's ledger through the same engine (D72), 81 new, and every one additionally required to *appear in the chapter that declares it* (D73), which is what stops a ledger being padded. **All 18 build guards are broken on purpose on every build** (`s25_test_guards.py`, D74) in a sandboxed copy, each required to fire with its own message, with the suite verifying it altered no committed file. **437 constructed negative controls across 19 modules**, derived from the test modules rather than counted (`thesis/control_inventory.tsv`). Also fixed: `s0_report.py` was computing the review's bibliography size as the size of the shared reference table, which would have silently restated 137 as 195 on the next render. → `thesis/`, `thesis/itpr_family_thesis.pdf` Brief: `docs/session_briefs.md`. *What the task started from:* the manuscript is the **compressed** form of 109,243 words of committed task reports, 68 recorded decisions and a 137-reference review, and cites 29 references. Three things it had to leave out are each a chapter: why each instrument is built the way it is (the Decisions log has never been written as prose), what was measured and abandoned, and the ~300 constructed negative controls as a body of work rather than a Methods sentence. **The references are the load-bearing half**: S0's rule is unchanged, so every new reference is audited on entry and committed as a table, because a bibliography that grows without an audit launders assumptions into citations. Builds on `s14_lib` — the claims ledger extends to the thesis, since a longer document is a larger surface for drift and not a licence for less checking. **Revised after the user read the assembled draft (D80):** every one of the 103 figure legends now closes with what the figure changes for a reader rather than only defending its own construction, and §15.1 states each new result against the literature it bears on, which took that section from 409 to 1,893 words and added 18 audited references, all 18 of which passed on the first attempt. Brief: `docs/session_briefs.md`. |
| S26 | **The paper series** — regroup every result into individual papers, each with its own question, controls, scope and figures | S14c, S24 | completed 2026-09-23 | **Six papers, 45,175 words, 122 pages, 702 load-bearing numbers in one ledger, every guard broken on purpose on every build (21/21).** The rules came first (`papers/paper_rules.md`, committed alone before any assignment) and the grouping was derived from them: range across the eukaryotes (S1–S3, S11, S20, S23; 26 pp); origin of the three paralogues (S6–S8, S13, S16, S21; 26 pp); retention across 309 genomes with its own false-negative rate (S4, S5, S15a/b; 19 pp); the archive audit (S5b's census v4, S10, S12, S18; 20 pp); constraint (S9, S17; 20 pp); and the ligand site (S22; 11 pp). **The brief's proposal changed in one place**: its constraint group split in two, because S22 has one question, its own controls (the six-deposition contact recovery, the RyR lineage control), its own scope and four figures, and passes P6 alone. **Every results entry is primary in exactly one paper or excluded with a reason** (38 entries, 73 rows, `papers/paper_assignment.tsv`), and the build fails on an unassigned one. Two directories are split by file under P5s because P2 forced it: S19's four halves are four papers' controls, and S24's supplementary figures draw three papers' data. **Four departures from the thesis's chapter assignment, each declared, and the cross-check between the two tables now exists** (the 2026-09-09 emergent row): methods, census v4, structures, supplementary. **P6 is enforced, not only written**: each paper's standalone answer names the claims it rests on, and each must come from a table primary in that paper. All six pass. The citation graph is read from `{paper:<id>}` citations, and it is acyclic and consistent with submission order (8 edges; range cites nothing). **Machinery generalised, not forked**: `s14_figures._copy_one`, `s14_deposit._notes`, `s14_refs.resolve` and `s14_claims.check` are shared unchanged or with a defaulted parameter, and the page checks are the thesis's (`s25_pdf.check_log`, extracted so the thesis's own glyph guard now calls the real function rather than a copy of it). **Three corrections to earlier documents** fell out of writing the papers: the deep constraint layer ranks third of four, not second (a word typed into `s17_report_variants.py`, now computed); the 1,232 genes behind the manuscript's database-reachability headline include the 309 RyR control genes (744/923 = 80.6 % for ITPR alone); and legends printed out of numerical order until the stitch sorted them (D85's defect, recreated and fixed at the source). → `papers/README.md` |
| S27 | **The editorial pass** — read every sentence of the thesis in context and hold it to four standards: factually correct, grammatical, meaningful, and useful to the reader. Check the document against IMRAD expectations, terminology consistency, citation coverage and cross-reference integrity | S25 | completed 2026-09-09 | **The pass found five classes of defect and fixed all of them.** **(1) Repetition introduced by the legend expansion**: 50 of 103 figure legends said the same thing twice, either inside the legend or against the paragraph beside it, because the *importance* sentence was appended rather than merged. All 50 were rewritten as single paragraphs and a detector for both forms is committed. **(2) One number wrong at source**: `s18_report_results.py` had 764 vertebrate reference proteomes typed into it, which is the line count of the manifest including its header, against 763 everywhere else in the package; it now reads `proteome_db_stats.json` (D13 applied to the one number the audit had typed). **(3) Four stale counts inside the thesis**: 75 decisions against 84, 26 sessions against 35, 58 added references against 76, and a chapter-16 anecdote about a correction that has since happened twice. Two new claims (T83, T84) now hold Chapter 14's figures to `production_stats.tsv`, so the same drift fails the build. **(4) Two wrong cross-references** in Chapter 6, both pointing a reader at the section before the one that answers the question. **(5) Six results chapters had no citations at all** — 5, 7, 8, 12, 13 and the search-worth half of 4 — so each now carries the literature its claims bear on, taking cited references from 111 to 116. Also added: the central research questions stated explicitly in §1.4, a **methods-location map** in §14.1 so a reader wanting a procedure rather than an argument can find it, a **data and code availability** section in the front matter, a practical-implications paragraph closing the abstract, and definitions at first use for the two remaining unexplained abbreviations. Verdict on the structure: the thesis meets IMRAD in substance with methods distributed by design rather than centralised, which is now navigable rather than merely defensible. → `thesis/` Recorded as **D81**, **D82** and **D83**. **Figure 1.1 was then replaced (D84):** the chapter's opening figure drew calcium event traces while its legend and the sentence citing it described the phosphoinositide pathway, so the first figure a reader met answered a question nobody had asked. `receptor_overview` replaces it with the object at three scales — the pathway as a schematic, the channel at the dimensions measured on 6DQN with its linear domain architecture, and the residues themselves at the two positions the structure marks — drawn by `s0_figs_overview.py` from the same committed S0 tables as the review's figures. The displaced figure moves to §1.3, where its content belongs, with a legend describing its own panels. |
| S28 | **The revision pass** — read the assembled thesis as a reader: legends and in-figure labels written as statements rather than noun-phrase fragments, every figure described in its chapter's text, legends typographically distinct in the typeset document, and every layout defect on the page found and fixed | S27 | completed 2026-09-23 | **Every rule the brief named now ends as a check, and the page read found six classes of defect none of them could see.** `s25_prose.py`: R1 (legend openers), R2 (every legend sentence), R1b (bold lead-ins) and R3 (every placed figure described by a body sentence of its own chapter), all through `prose_lex`'s finite-verb detector, **0 failures across 104 legends** (measured before S28: 83 of 104 openers, 33 of 369 lead-ins and 96 of 104 figures failed). `figcheck.py`, run by `figstyle.save()`: text overlap and clipping, fragments in panel titles and annotations, and, added after the page read, a title running into another panel, a title crowding the next panel's letter, a wrapped title's letter on the wrong line, and a legend drawn over its data; **all 102 figure stems render strictly** after 36 findings in 23 figures, and a second round of 19 found by a re-read and a tightened legend check, were fixed. R4: legends are a small ragged-right sans paragraph indented both sides, wrapped by the PDF stage, inside the same float as their figure. **The page read (D85)** found figures numbered out of order in six chapters (numbering now follows placement), stray rules at in-chapter file boundaries, half-empty pages ahead of pinned `[H]` figures (now `[!htbp]` with `\clearpage` per chapter), titles hidden behind neighbouring panels, legends over data, and a panel title contradicting its own panel. **A latent geometry bug since setup**: every figure is drawn for `s14_lib.W_FULL`'s 2.0 cm margin and both PDF stages set 2.2 cm, so 97 figures ran 11.9 pt into the margin; both documents are now 2.0 cm and the log check fails any overfull line over 4 pt. **30/30 guards fire** (twelve new), **234/234 claims**, thesis 190 pp, manuscript 60 pp with 276/276 claims. `thesis/figure_text_audit.tsv`: 333 changed texts, before and after. Brief: `docs/session_briefs.md`. |
...
| S15b | **The loss counts** — Dollo parsimony as the primary count plus Mk fits (ER / ARD / irreversible with gain pinned to zero) over the coding × evidence × branch-length × contiguity sensitivity matrix, and the pseudogene-fossil lesion analysis against a Poisson null | S15a | high | completed 2026-09-08 | **NEXT — and S15a changed what this task is.** With 0 `absent` cells the Dollo count is zero and the *sensitivity matrix is the deliverable*, not a robustness check: what has to be reported is which combinations of coding, evidence bar, branch lengths and contiguity filter **manufacture** a loss. `loss_candidates.tsv` is built for it — every near-miss row names the rule that stopped it. Three further constraints S15a hands forward: (1) the primary coding must be **family-level presence per genome**, with the paralog-resolved matrix as the sensitivity axis, because the `co_trace` population shows per-fragment paralog attribution is unreliable in a shattered assembly; (2) **Mk fits have no variation to fit** — an invariant character has no transition rate and reporting one would report the optimiser's start, so the informative version is the irreversible model on the matrix's non-degenerate cells; (3) **there are no pseudogene fossils** — every locus above the lesion bar is at full coverage with an intact model, so the shared-lesion Poisson test has no dead loci and must be reported with its denominator, with the ITPR3 indel excess (D47) as the lead to follow instead. **The count is 0, and the deliverable is the map of fragility around it.** Dollo places **0 losses** on the family character and 0 on each of ITPR1/2/3, across 927 genome × paralog cells in 309 genomes — and the routine that returns that zero is one that finds a constructed loss on a known edge (T1), merges sister losses into their parent (T2) and bounds a polytomy's losses rather than counting them (T4). **The sensitivity matrix is the result**: 32 settings (8 evidence rungs × D45 on/off × D4 on/off) × 2 codings × 3 branch-length schemes. The **primary family-level coding manufactures a loss in 2 of 32 settings**, worst case 1 genome in 309, and only by refusing everything but a complete locus *and* ignoring D4's bar; the **paralog-resolved coding manufactures one in 18 of 32**, up to 45 loss edges (37 independent). D46 was a judgement in S15a and is a measurement now (**D48**). Moving the reconstruction bar across the whole gap its calibration measured (0.100–0.174) changes **no cell in 927**; turning D45 off alone manufactures the 4 cyclostome losses immediately; tightening the evidence alone manufactures **exactly one** cell in the whole scope — *Bothrops jararaca* ITPR2, reassembled at 0.796 across 6 contigs, the single cell whose presence rests on the reconstruction instrument alone. The branch-length axis changes a Dollo count in **0** of 32 settings and is reported as invariant rather than dropped (D48). **Mk is stated, not fitted**: every likelihood is monotone to its boundary on every model, axis and scheme — measured on a rate grid, with ARD's *gain* axis rising to the edge — so all 12 fits at the operating point are refusals; the 405 fits that are possible are fits to manufactured losses and their rate spans a factor of 495 across the three branch-length schemes. **No pseudogene fossils**: 44 of 1,760 scored loci clear the lesion bar and **44 of 44 are at full coverage**, so the shared-lesion Poisson test has no dead loci and is reported with its denominator. **D47's lead is now a bird result** — ITPR3's indel excess is 25 genomes to 2 in Aves (q = 4.5e-5) and 7 to 6 in Actinopteri (p = 1.0) — but 21 of its 27 pairs are below D4's contiguity bar, so the verdict is `underpowered` and the siblings' deficits are the same comparison read from the other side (**D49**). Bonus: Dollo's unpinned gain nodes land at **Vertebrata** for ITPR1 and **Gnathostomata** for ITPR2/ITPR3 — S13's placement, from an instrument that reads no gene tree. 21 constructed controls pass before anything is written; 3 mutations applied, all 3 caught. → `results/loss_counts/report.md` |
| S16 | **Duplication history** — are ITPR1/2/3 2R ohnologs; are teleost itpr1a/itpr1b from 3R; copy-number landscape and retention asymmetry | S7, S8, S13 | high | completed 2026-09-08 | **Both questions answered, both with the RyR trio as the positive control.** **2R:** the ITPR1 neighbourhood is paralogous to ITPR2's in 141/175 genomes (80.6 %, 10 classes) and to ITPR3's in 89/152 (58.6 %, 9 classes) against **2.6 %** of 932 matched random neighbourhoods in the same genomes (p 4.9e-118, 9.4e-66); ITPR2 vs ITPR3 is 4/149 (2.7 %), exactly the background (p 0.55). Compara's duplication node then splits the two links: `GRM7 ↔ GRM4` (ITPR1–ITPR3) is **Vertebrata** and replicates in 84 genomes, `BHLHE40 ↔ BHLHE41` (ITPR1–ITPR2) is *Opisthokonta* and is **not** a 2R ohnolog pair — which S8 found and could not date. Pooled per family at ±10, ITPR 1 dated link vs null 0.060 (p 0.039) against the RyR control's 1 vs 0.022 (p 0.021): the two families are indistinguishable, so the human single-genome test measures the instrument's ceiling, not a difference. **3R:** ITPR1 alone was doubled and retained — 97.3 % of 73 teleost genomes above D4's bar carry 2 (mean 1.97) against 1.04 for ITPR2 and ITPR3, while the **same genomes carry 5.82 RyRs**; the pre-3R ray-fins (bichir, gar, bowfin) carry 1/1/1 + 3 and the extra-WGD lineages (paddlefish, salmon) 3/2/2 + 8. The two copies partition the ancestral block **disjointly** in 45/49 genomes (tetrapod reference) and 46/49 (pre-3R ray-finned), and all **705 of 705** cross-anchor assignments agree (p 1.2e-212) with 6/6 independent sequence-evidence concordance — one ancestral duplication, not a series. Split-model merge fired on 0 of 2,146 loci; 31 negative controls; the quartet test's 6 circular pairs are flagged in the data and 0 of 59 non-circular pairs are enriched. → `results/duplication/report.md` |
| S17 | **Constraint & function** — per-site conservation mapped onto the cryo-EM channel; do the SCA15/SCA29/Gillespie, anhidrosis and neuropathy variants sit in the constrained core? Is the IP3-binding core more constrained than the pore? | S6, S9b, S11 | high | completed 2026-09-08 | **The mechanistic payoff, and one element nobody had named.** **Which parts cannot change:** the gate and the selectivity filter are the most constrained elements of the receptor on two metrics in all three paralogs, and **the gate's five residues are 100 % identical between all three human copies** — RIH-associated is the most constrained *domain* (JSD 0.798–0.804). **The ligand site is not where Pfam says it is:** none of the ten measured IP₃ contacts (6DQN, ≤ 4.5 Å) lies in PF08709, the signature named *Inositol 1,4,5-trisphosphate/ryanodine receptor* — they sit in MIR and RIH, the domains shared with RyR — and those contacts are more constrained than the rest of the domains carrying them (p = 0.013 / 6e-4 / 4e-3). **The channel domain hid an element:** unresolved, PF00520 read as *less* constrained than the linkers (JSD 0.697, p = 0.96), which survived a metric control and a gene-model control; a 50-residue **luminal loop located by geometry** on 6DQN's own membrane span (ITPR3 2401–2450) is the cause — the least conserved element in the receptor (JSD 0.45–0.49, paralog identity 0.13–0.31 against 0.64–0.70 whole-protein, FEL median β 0.33–0.40 with 27–33 % of sites purifying against 71–86 % protein-wide) — and with it separated the pore module clears the linker control (p = 0.005). The most and least constrained sequence in the receptor sit ~50 residues apart in the same Pfam domain. **The variant test:** 1,753 ClinVar missense records, **0 dropped** by the transcript-numbering check (all three genes file on the canonical), both of S0's residue-level citations recovered by the positive control; **1,546 (88 %) are VUS** and ITPR2's entire pathogenic missense record is one variant. Constraint separates P/LP from B/LB — and **against this task's own design the family-wide layer wins**: on one fixed set of 44 vs 34 positions, AUC family 0.872 > vert 0.854 > **deep 0.758** > shallow 0.684, so the 249–265-orthologue sets S17 built are the third of four (S26 correction: the text said second-best for three editions while the table ranked it third). FEL: 5,766 purifying, **1 diversifying** site inside BH's own error budget. → `results/constraint/report.md`, `constraint_<gene>_<acc>.tsv` (per-residue, four layers), 16 painted structures, 4 figures, 14 self-tests (T8 caught `nterm_trefoil` being swept into the within-protein control). |
...
| S22 | **Ligand-site evolution** — the IP3-binding core is the one part RyR does not share functionally. Is it under different constraint from the pore, does it differ between paralogs, and does it change in lineages that lost the upstream PLC/IP3 pathway | S9b, S17 | medium | completed 2026-09-09 | **The module that names the family is the less constrained of the two, and the answer reverses on a boundary nobody states.** Both modules are defined by measurement rather than taken from Pfam: the **ligand core** is the minimal span holding every IP₃ contact S0 measured (265–569), the **pore module** is PF00520 **less** the luminal loop S17 located by geometry; each is checked against something it does not contain and a failure raises. **Paired per orthologue** (one core and one pore number per sweep orthologue, 246–262 per paralogue) the **pore is more conserved**: −0.0239 identity in ITPR1 (223/32 tips, q = 3.6e-34), −0.0199 in ITPR3 (218/42, q = 1.6e-28), nothing in ITPR2. **Leave the luminal loop inside PF00520 — as InterPro draws it — and all three flip to core > pore at q < 1e-37.** The column-level JSD sees no difference; the composition-free `frac_modal` agrees with the paired test, so the null is a property of the metric (JSD is a divergence from a background table and the pore is transmembrane). **The constrained unit is the pocket, not the contacts**: the ten contacts beat the rest of the core (q = 0.048 / 0.017 / 0.041) and beat the rest of the 15 Å pocket in none (q ≥ 0.12); every shell out to 15 Å sits above the whole-protein mean and there is no step at 4.5 Å. FEL replicates it — 100 % of contact sites purifying in all three paralogues, falling 0.13–0.25 across the shells. **The instrument:** every residue within 15 Å of IP₃ measured **all-atom** in **6** independent IP₃-bound human ITPR3 depositions; S0's ten contacts recovered in all six as a hard-failure positive control, and the consensus set is **12** — Ala276 and Arg411 are contacts 6DQN alone cannot see. **The lineage question is answered, not deferred.** The taxon list is derived, not read: PI-PLC (PF00387 **and** PF00388) swept over all **3,527 eukaryotic reference proteomes** → **64 carry an ITPR and no PI-PLC**, concentrated in the oomycetes and Mucoromycota, every one of them holding a full-length receptor. Pooled, their ligand core looks relaxed (p = 9.7e-6) — and that is a **clade artefact**: they sit at median pore identity 0.358 against 0.589. **Matched on divergence all 36 find controls and the effect is gone**: median within-pair difference −0.0064 (95 % CI −0.016 to +0.015), 19/17, p = 0.87, against a **RyR positive control measured on the same instrument at the same divergence** whose shift is −0.062 and which the matched test detects with ~100 % power. A bounded null. 32 tables, 4 figures, 44 negative controls, 10 mutation tests all caught → `results/ligand_site/report.md` |
| S14c | **Manuscript rewrite pass** — one full pass over the draft once every analysis has landed, with the figure audit's lessons applied | S14a, S24 | medium | completed 2026-09-09 | **Two completed analyses were not in the paper, and the figure numbering was not in text order.** S21 and S22 both landed after the S14a build and nothing in the draft referred to either, so the pass is a rewrite and not an edit: the S14a draft is frozen in `manuscript_v1/` (D2's practice, taken from PIEZO) and the paper gains a Results section. **The new section is the one the whole project's central control has been waiting for**: the three paralogues share a **median 46-49 of ~58 intron positions in every one of 181-183 genomes** at 85-88x chance, and the ryanodine receptors — which carry every ITPR-diagnostic Pfam domain and are inside every search here — share **one**, in **0** of 183-188 genomes. The sister family shares a fold, a pore and four signatures and has no exon structure in common, from a character no protein alignment can see. S22 folds into the machine section (the pore is more conserved than the ligand core in ITPR1/ITPR3 and level in ITPR2, and **all three reverse** on whether the luminal loop counts as pore; the constrained unit is a 15 Å pocket, not the ten contacts; losing the upstream PI-PLC is a **bounded null**), S21's fragment-terminus result into the annotation section, and both into the abstract, the discussion, the limits and the methods. **Figure numbering was wrong and is now checked by code.** Extended Data grew 14 → 16 and was renumbered into strict order of first mention, which it was not in: **ED 2 and ED 3 were cited by no sentence at all** and the methods figure was numbered last and first cited in the third section. `s24_audit.citation_order()` now enforces cited-and-in-order on every build, mutation-tested four ways. **Three defects found in the build itself, all fixed.** (1) `gene_architecture` and `ligand_site` were missing from `DEPOSIT_DIRS`, so two completed tasks' results were **not being deposited**. (2) The reviewer checklist claimed the build reads the LaTeX log for `Missing character`; **it did not** — pandoc reports only its own warnings unless asked — so `s14_pdf.py` now runs `--verbose`, scans the log and exits non-zero on a dropped glyph (mutation-tested; currently zero). (3) The checklist itself, the one hand-typed artefact left, was four editions stale (175 claims against 180), so `s14_claims.py` now fails the build unless its headline count is the ledger's. One number was corrected against its table: the fall in purifying selection across the ligand pocket is measured to each paralogue's **lowest** shell, not its furthest — the outermost sits above the third in all three, so it is a step onto a floor and not a gradient. **The lead was re-examined and left alone**: the title's two halves are still the paper's two strongest results, and both new analyses are about what the gene and the protein are like rather than where they are. Claims **180 → 276**, all passing; 17 sections, 17,807 words, 60 pages, build green end to end. → `manuscript/`, `manuscript_v1/FROZEN.md` |
...
|-------|------|------|--------|
| 2026-09-23 | S26 | **Two ligand-site tables disagree about the size of the PI-PLC panel.** `pathway_cooccurrence.tsv` sums to 3,520 proteomes, while `plc_repertoire.tsv` holds 3,527. The ligand paper reads the repertoire table and says so, but the seven-proteome difference is unexplained, and S22's report quotes whichever table its paragraph was rendered from. | open |
| 2026-09-23 | S26 | **The appearance rule cannot read a percentage, scientific notation or a trailing `.0`, so some numbers are declared twice.** `s25_claims._renderings` matches 1,236 for 1236 and 0.5 for 0.50, but not 20.9 % for 0.209, 1e-08 written as 10⁻⁸, or 147,445 for 147445.0. Three paper writers worked around it by re-declaring a manuscript claim at the printed precision with a tolerance (for example C62 and OR10), which the declared-once signature check cannot see because the expected values differ. Widening the renderings and then collapsing the pairs would make "checked once" true without exception. | open |
| 2026-09-23 | S26 | **The manuscript ledger has no row for its corrected database-reachability headline.** S26 corrected the abstract to 744 of 923 IP₃ receptor genes. The number is checked in the archive paper's ledger (from `results/methods/gene_recovery_by_cell.tsv`), but not in `s14_claims`, whose C168–C172 still check the 940/1,232 figure. Those claims now describe a count the abstract no longer states. Adding the row changes the manuscript's claim total, which `reviewer_checklist.md` quotes and the build checks. | open |
| 2026-09-23 | S28 | **An annotation printed over a plotted curve is still invisible to every check.** `figcheck` now refuses a legend drawn over its data, but a free annotation over a line is legitimate as often as not (a label on its own bar, a leader-lined note), so it was fixed by hand where the S28 page read found it (`channel_structure` c) and is not checked. A rule that separates a label placed *on* its mark from one printed *across* another mark would close it. | open |
| 2026-09-23 | S28 | **The manuscript's PDF stage has none of the thesis's layout checks.** `s14_pdf.py` shared the 2.2 cm margin bug and now shares the fix, but it does not read the LaTeX log for overfull lines or tall floats and no page-by-page read of the 60-page PDF has been done since the figures were revised. S26 generalises this machinery to N papers and should carry the thesis's checks with it rather than the manuscript's. | **half done (S26)** — the six papers are typeset with the thesis's checks (`s25_pdf.check_log`, the legend typography, floats bound to their legends) and were read page by page. The single manuscript's `s14_pdf.py` still has none of them. |
| 2026-09-23 | S28 | **Nothing checks that the scripts parse on the project's own interpreter.** Two committed modules used multi-line f-string expressions, which only Python 3.12 accepts, and the build failed in the documented 3.11 env (D18) only when S28 happened to run it there. A stage that `ast.parse`s every script under the env's interpreter would have caught both on the day they were written. | open |
| 2026-09-09 | S25 | **The typeset PDFs are reproducible in content and not in bytes, and nothing records the difference.** D65 made every *figure* pdf byte-identical across rebuilds by dropping its creation timestamp. S25 applied the same to the document: `SOURCE_DATE_EPOCH` now fixes the build date in `s25_pdf.py`, and two builds of the thesis differ in exactly 64 bytes of 3.5 million — two copies of a random 16-byte trailer `/ID` that this xdvipdfmx writes regardless of `FORCE_SOURCE_DATE`. So a SHA-256 recorded against a typeset pdf still means nothing, while a checksum over the *content* would. `s14_pdf.py` has the same defect and does not set `SOURCE_DATE_EPOCH` at all. Either both stages should normalise the trailer after the build, or the manifests should record a content digest and say so. | open |
...
| 2026-09-08 | S19 | **The panel could be four baits, and that changes what a bait budget should buy.** S19's exact ablation shows four human baits recover 782 of the 783 cells the 38-bait panel recovers, and that dropping any single clade band costs at most two. The 38-bait panel was built by seven enforced rules aimed at phylogenetic spread; the measurement says spread within a paralog is worth almost nothing at vertebrate depths and that the binding constraint is paralog coverage plus the clades with no labelled record at all. Two consequences worth a session. (a) **S23's non-vertebrate panel is 68 baits chosen by the same logic across a far greater depth**, where the identity gap to the nearest bait is much larger — the same ablation run on the S23 sweep would say whether breadth is free there too, and there is every reason to expect it is not. (b) A four-bait vertebrate panel would run the 309-genome sweep roughly ten times faster, which is the difference between a 19-hour sweep and a two-hour one for anyone replicating this. | open |
| 2026-09-08 | S19 | **A one-line change to D10's kill criterion separates every jackhmmer run this project made, and it is not applied.** Moving K1's own 0.10 threshold from the sister-family share to the off-family share gives sensitivity 1.00 and specificity 1.00 over seven runs where K1 gives 0.00 and K3 gives 0.25 specificity (D59). Applying it would overturn the committed verdicts in `census_v3_stats.json` and `jackhmmer_verdicts_s20.json` — `s20_metazoa_nonvert` and `itpr1_human` would move from `killed` to a run that simply did not finish, which is what S20b already says in prose. That is a decision about published tables, not a methods result, and it needs a session that owns them. Seven runs cannot establish a specificity of 1.00 either; the honest next step is to score the rule on the PIEZO project's twelve runs, which are independent and already parsed. | open |
| 2026-09-08 | S19 | **Three quarters of this family's genes are unreachable from any protein database, and the correction list addresses almost none of them.** 940 of 1,232 demonstrated genome x cell genes have no protein record that resolves to them: 386 species with no reference proteome at all, 248 whose only family records are fragments, 286 where records exist and none is assignable to that paralog. S18's `corrections.tsv` is addressed to genome annotators about loci; this is a different deficit, addressed to protein resources, and it is the larger one. The submittable form is not obvious and needs deciding before S14a: a bulk UniProt/UniParc submission of the 1,058 census v4 gene models, a supplementary FASTA resource, or a statement in the paper and nothing more. | open |
| 2026-09-08 | S19 | **The ablation is silent about tblastn rescue, which is where S5a's known bait incident actually lived.** S19 reproduces the sweep's locus placement 1,236/1,236 cells and measures the panel against that, but rescue attribution needs the genome FASTA, which the sweep deleted after searching. S5a's `vertebrate_basal`-as-fourth-paralog incident and the `ATTRIBUTION_REL_MARGIN` recalibration (D25) both belong to that path, and S19 can say only that the unlabelled baits change no *miniprot* cell. Re-fetching the 44 genomes that carry a trace cell would close it at roughly 25 GB and a few hours; whether that is worth it depends on whether S14a wants to claim the panel was validated end to end or only for placement. | open |
| 2026-09-08 | S19 | **The 57 false negatives whose neighbourhood is also missing are a regional-assembly class nobody has looked at directly.** S19 asked S8's consensus flanks whether the region survived at all in each of the 182 control-cell misses, and in 57 of them fewer than half the paralog's usual neighbours are in the assembly. Those cells are not evidence about the gene and no contiguity statistic can see them — contig N50 is genome-wide and these are local. The PIEZO project met the same thing once (a chromosome-level tenrec missing an entire orthologous segment) and treated it as an anecdote. With 57 cases here it is a category, and `residual_neighbourhood.tsv` is the list. Worth one pass to see whether they cluster by assembly, by submitter, or by genomic region. | open |
| 2026-09-08 | S21 | **The exon structure does not explain D47's ITPR3 indel excess, and S21 narrows the readings from three to one.** S15a left three readings of ITPR3's disabling-indel excess and named S21 as the task that could settle the second — that ITPR3's exon structure is systematically harder to align through. It is not supported: ITPR3 has the **same** exon count as ITPR1 (58) and the **shortest** introns and genomic span of the three (357 bp median, 58 kb), which should make it easier to align through, not harder. But the excess is visible in the alignments themselves and survives the contiguity control: in `junction_quality.tsv`, restricted to complete models in assemblies that can hold the gene, ITPR3's junctions carry a frame step at **0.0101** against ITPR1's **0.0051** and ITPR2's **0.0080**, with the RyR control at 0.0080. So the assembly reading and the exon-structure reading are both weakened and reading 1 — real indel tolerance in ITPR3's coding sequence — is what is left. The paired, identity-matched version of that comparison is `s15_integrity.py` run unchanged on `architecture_loci.tsv`'s `frame_steps` column, which is a few lines and no new data. | open (S22) |
| 2026-09-08 | S21 | **Intron positions are a character matrix and nobody has polarised it.** S21 shows the three paralogues share ~48 of ~58 intron positions and that each has 48-49 positions present in >=90 % of its genomes — but it counts *sharing*, not *history*, so a position present in ITPR1 and absent in ITPR2 could be a gain on one branch or a loss on the other, and S21 says so in its own §11. Everything the reconstruction needs is now committed: `intron_positions.tsv` gives every locus's positions as (column, phase), `results/loss_dynamics/species_tree_309.nwk` places every swept assembly, and `s15b_dollo.py` already implements Dollo parsimony with the reachability self-tests. The interesting cell is the ~10 positions per paralogue that are *not* shared, and whether they are concentrated in one lineage or one part of the protein. | open (unassigned) |
| 2026-09-08 | S21 | **The non-vertebrate exon architecture is one parse away, and the blocker is the frame rather than the data.** S21 is a vertebrate result because its coordinate frame is the three human paralogues. The S23 sweep's **194 non-vertebrate genomes retain both their `miniprot.gff` and their FASTA**, so the same instrument — blocks, splice dinucleotides, exon counts, intron positions — runs unchanged over them and would answer whether the ~58-exon architecture predates the 2R duplications, which is currently unanswered. What it needs first is a frame decision: S23's baits are labelled by clade band and not by paralogue (B1, deliberately), and `msa_v2` carries no non-vertebrate reference to anchor columns on, so the shared-position test would need either a new anchored alignment or to be restricted to per-genome exon counts and intron lengths, which need no frame at all. The second half is free and worth doing first. | open (unassigned) |
| 2026-09-08 | S21 | **353 annotated model termini sit inside an intron of the gene model, and no correction list carries that class.** S18's 297 corrections are about coverage, state and naming; S21's fragment test found a different disagreement — the annotation calls coding what the alignment calls intronic, at 353 of 1,448 internal termini across the 291 `split`/`fragmentary` loci, recorded as `structure_disagreement` in `fragment_verdicts.tsv` (46 loci carry it and no mid-exon terminus). Some of those will be the annotation right and the alignment wrong, which is exactly why the verdict does not adjudicate. They are coordinate-level, per-terminus and archived, so they are submittable in the same form as S18's rows if anyone decides they should be. | open (S14b) |
| 2026-09-08 | S21 | **Why ITPR2 needs four times as much chromosome as ITPR3 for the same protein is open, and the sequence to answer it is on disk.** The three paralogues carry near-identical coding sequence (8,008-8,250 bp) in 58 kb, 147 kb and 244 kb of genome, a 4.2-fold spread that holds gene by gene inside genomes rather than on average. S21 measures the pattern and offers no cause. The first question is whether the extra intronic content is repetitive — the 309 genomes and every locus's intron coordinates are committed, so a repeat-content comparison across the three paralogues' introns within each genome is a paired measurement needing no new data. The second is whether the ordering is ancestral or has drifted, which is the same character-matrix job as the intron-polarisation row above. FINDINGS marks this *(pending: no task in this plan tests either reading)* and it should have a row. | open (unassigned) |
| 2026-09-09 | S22 | **The IP₃ site has twelve contact residues, not ten, and one of them is an invariant arginine nobody in this project has named.** S0 measured the contact set on 6DQN alone at ≤ 4.5 Å and got ten; measured across six independent IP₃-bound human ITPR3 depositions the consensus set is **twelve**. Ala276 (median 4.27 Å, inside 4.5 in 4 of 6) and **Arg411** (median 4.34 Å, 4.05-4.57 Å in every structure, inside 4.5 in 4 of 6) are contacts that one map alone cannot see. Arg411 is invariant across 265 ITPR3 sweep orthologues (`frac_modal` 0.996), sits in the MIR domain, and maps to ITPR1 Arg412 / ITPR2 Arg411 — three arginines and two lysines already make the measured contact set, so a sixth basic residue at 4.3 Å from a trisphosphate is chemically unsurprising and structurally load-bearing. Everything that quotes "the ten IP₃ contacts" — `functional_sites.tsv`, S17 §6.3, the review's ligand figure, the manuscript — is quoting a one-structure number. The fix is a rerun of `s0_figdata_structure.py` over the replicate panel and a decision about which set the family's tables carry. | open (unassigned) |
| 2026-09-09 | S22 | **64 eukaryotic proteomes hold an IP₃ receptor with no phosphoinositide-specific phospholipase C, and nobody has asked what opens them.** The list is not scattered: 22 of the 28 ITPR-carrying fungal proteomes (all Mucoromycota and chytrids), 20-odd oomycetes, three *Perkinsus*, two ciliates, four prasinophyte algae, and a group of platyhelminths including four *Schistosoma*. Every one carries a full-length receptor, so this is not an annotation gap. S22 shows the ligand core is **not** relaxed in them, which makes the site's ligand an open question rather than a vestige. The next measurement is cheap and uses machinery that exists: sweep those proteomes for the other routes to a soluble inositol phosphate — bacterial-type PI-PLC (PF00882), the PLC-like phosphodiesterases, and the inositol polyphosphate kinases — with `s22_plc.py` unchanged but a different profile list. | open (unassigned) |
| 2026-09-09 | S22 | **Two of this project's own committed measurements disagree about ITPR1's pore and nothing reconciles them.** S22 §6 has the pore module more conserved than the ligand core in ITPR1 by 0.024 identity, paired over 260 sweep orthologues (q = 3.6e-34); S17's FEL rates have the same pore module evolving *faster* than the same ligand core in the same paralogue (mean β 0.0446 against 0.0251, q = 6.2e-8). Both are computed correctly. The likely reading is the obvious one — 57 vertebrate tips against 260 orthologues, a rate on a tree against a column's dispersion — but "likely" is not a measurement, and the check is one run: FEL, or simply a per-orthologue paired identity, on the same 57 tips both instruments could share. If they still disagree it is a real difference between amino-acid-level and codon-level constraint in a transmembrane module, which is worth its own paragraph. | open (unassigned) |
| 2026-09-09 | S22 | **The Mucoromycota cell has no internal control and the claim about it is a phylum-level one.** All 15 Mucoromycota reference proteomes that carry an ITPR lack a PI-PLC, so S22's within-phylum test cannot run there at all — the phylum contributes to the pooled result and to none of the stratified ones. That is either the strongest case in the list or an artefact of how few Mucoromycota reference proteomes exist, and the two are separable with S23's machinery unchanged: a targeted genomic sweep of Mucoromycota assemblies for both the receptor and the two PI-PLC domains, which would give the phylum a denominator that is not UniProt's reference set. | open (unassigned) |
| 2026-09-09 | S22 | **The per-orthologue paired design is a general instrument and S17's element ranking has not been through it.** S17 ranks every element on per-column statistics over the deep alignments; S22 shows that for a comparison crossing a membrane boundary the per-column JSD and the per-orthologue paired identity give different answers, and that the composition-free metric sides with the paired one. Re-running S17's whole element table as a paired per-orthologue contrast is a few lines against files that are already committed (`aln_<gene>.fasta`, `domain_map.tsv`, `s22_paired._pairs`), and it would say which of S17's element rankings survive a design that cannot be moved by amino-acid composition. | open (S14c) |
| 2026-09-08 | S17 | **A 50-residue ER-luminal loop is the most variable sequence in the receptor and nothing in this project has looked at it.** Located by geometry on 6DQN (ITPR3 2401–2450; ITPR1 2481–2531; ITPR2 2427–2474), it is the least conserved element on every instrument S17 has: JSD 0.45–0.49 against a 0.73 linker mean, paralog identity **0.13–0.31** against 0.64–0.70 whole-protein, FEL median β 0.33–0.40 with only 27–33 % of sites purifying, and trimAl deletes 39 of its 51 residues. It is also the region the cryo-EM maps resolve worst. Two questions follow and neither is S17's: is it the luminal Ca²⁺/ERp44-sensing insert the literature places between TM5 and the pore helix, and is its length variable across the 309-genome scope (it would show up in S21's exon structure as a single variable exon)? A paralog-specific luminal insert next to an invariant gate is a functional-divergence hypothesis with a testable sequence signature. | open |
| 2026-09-08 | S17 | **The family-wide conservation layer beats the deep within-paralog one as a variant classifier, and every downstream task should quote it.** On one fixed set of 44 pathogenic and 34 benign positions the AUCs are family 0.872, vert 0.854, deep 0.758, shallow 0.684 — so a position conserved across the eukaryotes discriminates better than one merely invariant across 260 vertebrate orthologues of the same gene. S22's ligand-site work and any variant resource this project publishes should read `family_jsd`, not `deep_jsd`; S24's supplementary figures should show which layer a claim rests on. The deep layer is still what §5's element ranking needs (it is the only layer with per-paralog resolution), so this is a per-question choice and not a retirement. | open |
| 2026-09-08 | S17 | **ITPR2's clinical record is one pathogenic missense variant, and S17 turns that into a falsifiable prediction.** ClinVar holds 299 ITPR2 missense records: 1 P/LP, 11 B/LB, 261 VUS. Its gate is identical to ITPR1's and ITPR2's, its ten IP₃ contacts are the *most* constrained of the three paralogs (JSD 0.840, 8 of 10 invariant across 249 orthologues), and its pore module clears the linker control as well as ITPR1's does. On this evidence the emptiness is ascertainment, not tolerance — ITPR2 has no dominant paediatric syndrome drawing sequencing to it — and the prediction is that missense variants at ITPR2's gate, filter and IP₃-contact positions will be pathogenic when anyone looks. The 196 scored ITPR2 VUS are where to look: `vus_stratification.tsv` puts 6 % of them at or above that gene's P/LP median. | open |
| 2026-09-08 | S17 | **1,546 uncertain variants now carry a per-site score, and that is a submittable artefact this project has not planned for.** `variants.tsv` joins four conservation layers, the FEL rate, the element and the measured-site flags onto every harvested variant, and `vus_stratification.tsv` places each VUS against its own gene's labelled distributions (10–11 % of ITPR1/ITPR3 VUS sit at or above the P/LP median). S14a's deposit rules do not currently mention a variant table, and S18's correction list is about annotation rather than variants. Deciding whether the manuscript offers this as a resource — and with what disclaimer, since a stratification is not a call — is a scope question for S14a rather than a further analysis. | **decided (S14a)** — offered in the supplementary section with the disclaimer stated in the paper's own words: it says where a variant sits on an axis that separates the labelled ones and nothing about its effect; the tables are in the deposit |
| 2026-09-08 | S17 | **The gate cannot distinguish the paralogs, and one D14-adjacent consequence is worth recording.** The five gate residues are 100 % identical across ITPR1/ITPR2/ITPR3 and the selectivity filter is 0.86–1.00, so no pore-based feature separates the three copies — every paralog call this project makes rests on sequence outside the conducting machinery. That is consistent with D14 (the family call is architecture, not the pore) but it also means a future structural or pore-motif shortcut for paralog assignment is unavailable in principle, not merely untried. Worth one line in the methods beside D14. | open |
| 2026-09-08 | S15a | **ITPR3 carries a disabling-indel excess that nothing in this project explains, and it points the opposite way to the constraint result.** Within one genome, against sibling family loci matched on bait identity, ITPR3's recovered gene models carry more frameshifts and in-frame stops than ITPR1's or ITPR2's — 39 genomes to 14, q = 0.0032, with the RyR control showing no excess and ITPR2's apparent excess dissolving under identity matching (D47). That is hard to reconcile with two earlier results: S9b has ITPR3 under *looser* purifying selection than ITPR1 (so the codon-level and indel-level pictures agree in direction), but S5 has ITPR3 as the **best**-recovered and best-annotated paralog (232 `found_annotated` cells against 194 and 170). Three readings worth separating, and each is testable with data already on disk: the excess is real indel tolerance in ITPR3's coding sequence; or ITPR3's exon structure is systematically harder to align through (S21 measures exon architecture from the same miniprot CDS blocks and could settle it); or the effect is driven by one clade rather than being family-wide (the per-genome pairs are committed in `integrity_pairs.tsv` and can be stratified by vertebrate class without any new computation). **S15b ran the third reading and it fired: the excess is a bird result** — 25 genomes to 2 in Aves (q = 4.5e-5) against 7 to 6 in Actinopteri (p = 1.0), so it is not family-wide, and the sibling deficits in the same table are the same within-genome comparison read from the other side (D49). **S21 ran the second reading and it does not hold**: ITPR3 has the same exon count as ITPR1 and the shortest introns of the three, so its structure is not harder to align through, while its junction frame-step rate stays twice ITPR1's among complete models in contiguous assemblies. Reading 1 — real indel tolerance — is what is left, and the bird finding is `underpowered` for the reason in the row below. | partly answered (S15b, S21); reading 1 open (S22) |
...
| 2026-09-08 | S16 | **S8's paralogon headline needs one qualification before it is quoted again.** S8 §4 reports that the surviving 2R paralogon links run through ITPR1 — one retained flank family with ITPR2 (`BHLHE`) and one with ITPR3 (`GRM`). S16 confirms the shape with an independent instrument and then dates both links against Ensembl Compara: `GRM7 ↔ GRM4` is a *Vertebrata* duplication and `BHLHE40 ↔ BHLHE41` is *Opisthokonta* — a pair far older than the vertebrates whose two copies happen to sit beside ITPR1 and ITPR2. It is real paralogy and it is **not** 2R evidence (D51). S8's committed report and figures are not wrong and are not being rewritten, but any manuscript sentence built on "two retained ohnolog families" must say **one**, and the S8-derived figure in the S14a plan needs the same check. | **done (S14a)** — the manuscript says *one* dated 2R ohnolog family and states the `BHLHE` link's *Opisthokonta* date; Fig. 4 is S16's replication panel, which draws the dated links separately from the undated ones |
| 2026-09-08 | S16 | **Separating 2R round R1 from R2 needs an instrument that does not rest on gene symbols, and two tasks have now hit the same wall.** S8 reported cyclostome flank synteny as `underpowered` — 27–29 % of lamprey and hagfish coding genes carry an informative symbol at all — and handed the question forward; S16's dated paralogon reaches only *Vertebrata*, which spans both rounds, and hits the same limit from the other side. The three cyclostome loci per genome that S5 filed in the ITPR1 cell are exactly the evidence that would resolve it. Candidate instruments that avoid symbols entirely: Compara-style orthology calls on the flanking *sequences*, or the 2R paralogon reconstructed from a cyclostome-anchored gene tree — both named in S8 §5.3 and neither attempted. | open (unassigned) |
| 2026-09-08 | S15b | **The bird ITPR3 indel excess needs a different sample, not a different statistic.** S15b localised D47's excess to Aves (25:2, q = 4.5e-5, identity-matched, within genome) — but **21 of those 27 pairs sit in assemblies below D4's contiguity bar**, which is where the test has all its power (19:2, p = 2.2e-4), and the 6 above the bar all point the same way without being enough to test. S5b measured 66 % of Aves assemblies below the bar, the worst of any class, so this is precisely the class where an indel signal is hardest to separate from an assembly signal. The test is a bird panel restricted to chromosome-level assemblies — more contiguous bird genomes, scored with the same committed instrument. No new method is needed; `integrity_loci.tsv` and `s15_integrity.py` run unchanged. | open (S18 / S19) |
| 2026-09-08 | S15b | **One cell in 927 rests on the reassembly instrument alone, and nobody has looked at it directly.** *Bothrops jararaca* ITPR2 (GCA_018340635.1) is the only cell that reads `absent` when the evidence bar is tightened with **both** protective rules left on: its reference reassembles at 0.796 across 6 contigs in an assembly contiguous enough to carry the gene, with no spare family locus to explain it. It is not a loss under any bar the calibration licenses, but it is the single cell a sceptical reviewer should be handed, and the cheapest way to close it is the one already on disk — the six contigs, their order against the reference, and whether the gaps between them are N-runs or true assembly breaks. | open (S19 / S21) |
...
| 2026-09-08 | S12 | **The annotation-correction list now has three submittable cases with read-level evidence, and they are a *different* class from S18's usual ones.** *Nibea albiflora* ITPR1 and ITPR3 are annotated as pseudogenes over 0.98 and 0.97 of their coding footprint and are transcribed in 32/32 and 31/32 libraries; ITPR2 is unannotated and transcribed in 31/32. S10 had already shown all three splice into an uninterrupted reading frame with 0 internal stops. So the correction is not "extend a model" but "this gene is not a pseudogene", which is a biotype change that moves three proteins into the databases for a species that currently has none. The same test applies to the four *Dissostichus* loci. Evidence is committed per junction in `results/expression/junction_support.tsv`. | open (S18) |
| 2026-09-08 | S12 | **Deposit-based transcript evidence is structurally incapable for this family, and that is a per-method contribution number S19 should carry.** 854 junction probes against every mRNA record NCBI holds for the three species — including *D. mawsoni*'s 37,166 — return **0 spanning hits**, and 0 for the annotated RyR control the databases do name. The genomic negative control fires (1,791 hits, 0 spans), so the criterion discriminates. The cause is length: median deposit 547 nt against an 8 kb transcript. S19 asks "what would proteome-only searching have missed"; this is the transcript-side twin of that question, and the answer is that for a ~2,700 aa channel the deposits answer nothing whatever their count. | open (S19) |
| 2026-09-08 | S18 | **The non-coding demotion is this family's own annotation failure and nobody has looked at one directly.** An ITPR locus is 2.7x more likely than a RyR locus in the same assemblies to be held only by a non-coding feature (42 vs 16, q = 0.006) — a gene correctly identified and correctly named, filed as `gene_biotype=other` or `pseudogene`, serving no protein and unreachable by any name-based search. Above D4's contiguity bar the same comparison is 4 against 5 (q = 1.0), so the excess is confined to broken assemblies and this task cannot say whether it is a fact about the family; 54 of the 297 corrections are this class. S10's machinery (spliced ORF, stop count against the neutral expectation, junction probes) would settle whether the submitters are right about the frameshift they cite, and it has never been pointed at one. | pending |
| 2026-09-08 | S18 | **55 % of the family's full-length protein records have no usable gene symbol, and that is a per-method number S19 needs.** 3,872 of 11,402 carry a placeholder and 2,395 carry none, so a symbol-driven search of the protein databases reaches under half of what is there — the protein-side counterpart to S5b's 318 DNA-only models. S19's per-method contribution question is currently framed around sequence search; this is the same question asked of the *name*, and it is answerable off `protein_audit.tsv` without new data. | pending |
| 2026-09-08 | S18 | **The bait panel has no RYR3 bait and 74 records now depend on that.** 74 full-length vertebrate records carry a RYR3 symbol that this panel cannot call — it assigns them RYR2 confidently, at margins to 0.36, because the right answer is not in the panel. They are recorded as outside the instrument's reach rather than scored, which is honest and leaves the RyR control's paralog structure unaudited. A RYR3 bait is one `s5_bait_spec` slot away and would close it. | pending |
| 2026-09-08 | S18 | **The 297 corrections are a submittable artefact this project has not planned a route for.** Each row carries assembly, coordinates, strand, current state, current name, the proposal and an archived evidence file; 52 are `high` priority with the gene demonstrably present, the contig demonstrably able to carry it and the reading frame intact. S12 added three cases with read-level evidence. There is no ledger row for actually sending them, and S14b's deposit step is the natural place. | pending |
| 2026-09-08 | S14a | **The manuscript cites 29 of the project's 137 curated references and nobody has checked the other 108 against what the paper now claims.** The literature baseline audited 19 `[lit]` claims in S0 and the review cites all 137; the manuscript draws on 29, chosen while writing. Two gaps follow. The discussion asserts that the lineages missing the receptor have reduced or re-routed calcium signalling and cites three surveys, none of which was audited for that specific statement. And the paper makes no contact at all with the functional literature on paralog-specific channel behaviour beyond one sentence, although `references.tsv` holds ~20 rows on it. One pass with `s0_lit`-style discipline over the discussion's claims, using the table already in the repo, is the cheapest way to close both. | open (S14c) |
| 2026-09-08 | S14a | **The paper reports a 15.2 % false-negative rate and then reports results computed on the cells that rate applies to.** S19 measured it and S15b showed no loss is reconstructed, so the two together license the retention claim — but every *other* per-cell number in the manuscript (annotation states, copy number, lesion density, synteny) is computed on the same ledger and inherits the same miss rate without restating it. Above D4's contiguity bar the rate falls to 0.9 %, and most of those results are already reported both ways, but not all of them are and the paper does not say which. A single pass tabulating, per results section, whether its numbers are quoted over 309 genomes or over the 189 above the bar would make the caveat structural instead of a sentence in the limits. | open (S14c) |
| 2026-09-08 | S14a | **The figure set leaves 28 committed publication figures out of the manuscript entirely.** 84 figures exist across 21 results directories; 56 panels are placed. What is left out is mostly instrument and calibration work — the census-v2 enumeration figures, the S23 identity-floor and span-inflation panels, the S20 profile-separation and jackhmmer panels, three loss-count panels, the S12 detection and instrument panels. They are in the deposit and none is needed for a claim, but several are the *evidence for a method decision the paper states in prose* (the retired identity gate, the cluster-vs-gene copy rule, the D22 module floor). S24 owns supplementary figures and is the place to decide whether any of them should be promoted rather than merely archived. | open (S24) |
| 2026-09-08 | S12 | **The instrument reaches far more loci than the 7 it was pointed at.** S12's scope was S10's eligibility rules, which is right for validating *annotation failures* — but S5b's ledger holds hundreds of `found_unannotated` cells across the 309 genomes, and the streaming aligner costs ~135 s per run against a reference that takes seconds to build from an archived GFF. A second pass over the `found_unannotated` cells in species that have RNA-seq would turn a large block of the ledger from "recovered by one instrument" into "recovered and transcribed", which is what S18's audit and S19's methods results both want. The rate-limiting input is SRA availability per species, already measured for any species by `s12_runs.py --species`. | open (S18 / S19) |
| 2026-09-08 | S11 | **The family is effectively absent from AlphaFold DB, and that is a result the manuscript should state rather than a limitation to apologise for.** 13 of 5,861 census records at or above the 2,000 aa floor have a usable model (0.2 %); 9 of S6's 134 representatives do. Two consequences. (a) **S17 and S22 cannot be run over the family from public predictions** — they can be run on the ~20 full-length structures that exist (3 cryo-EM paralogs + a 6-state ITPR3 series + 11 AFDB models, all vertebrate or invertebrate-metazoan), and any per-site constraint mapped onto a structure is mapped onto a *vertebrate* structure. (b) If the project wants the non-vertebrate grade structurally, it has to **generate** the models: the AFDB records for Viridiplantae, Fungi, SAR and Discoba are 1,019–1,274 aa partials at mean pLDDT 60–68, and each reaches only 0.56–0.74 of the TM-score its own length allows. An AlphaFold 3 / ColabFold run over the S6 representatives is the obvious follow-up, and the PIEZO project already has the job-preparation code (`../piezo_genes/scripts/s11_af3_jobs.py`). | open (S17 / S22 / S24) |
| 2026-09-08 | S11 | **`in_alphafold` in census v6 disagrees with AlphaFold DB on 6 of 7,812 records, and agrees on the rest — but it cannot see the isoform problem at all.** UniProt's cross-reference is 99.92 % concordant with the live API on presence/absence, so it is a sound cheap proxy for *whether* a model exists. It says nothing about *what* is modelled: two of the three human paralogs are served as isoform records (ITPR2 at 181 of 2,701 residues), and the flag reads 1 for both. Anywhere the project quotes structural coverage it must quote the coverage-weighted number, not the flag. | open (S14a) |
| 2026-09-08 | S11 | **No full-length RYR3 cryo-EM structure exists**, so every structural statement about the sister family in this project rests on RYR1 and RYR2. Worth one line in the methods; it also means the RyR outgroup in any structural figure is a two-paralog outgroup. | open (S24) |
| 2026-09-08 | S10 | **The two validated loci should be in S12's panel.** S10's junction evidence is a real negative with a stated denominator — *Nibea albiflora* has 43 transcript records in the nucleotide databases and *Dissostichus eleginoides* 10, and neither has any TSA. But both have RNA-seq: 182 and 19 runs. S12 builds exactly the streaming aligner that reaches it, and its brief already wants a species panel where all three paralogs are present as internal controls — which is true of both. The probes are committed (`junction_probes.tsv`, with the junction offset and the anchor rule), so S12 can count junction-spanning reads over the same junctions rather than deriving new ones. | **closed 2026-09-08 (S12)** — both species are in the panel with *D. mawsoni* added as a third; the junctions were re-derived from the archived GFFs rather than read from `junction_probes.tsv`, and independently reproduce S10's count exactly (55 unannotated junctions for *Nibea* ITPR2). 298 of 314 are crossed by reads. |
| 2026-09-08 | S10 | **Two annotations file a third of their gene set as pseudogene, and the family is inside it.** *Nibea albiflora*: 7,380 pseudogenes of 23,345 genes (31.6 %), and **every complete IP3R and RyR gene model in the assembly is one of them** — the species has exactly one family protein record in any database, a 976 aa RyR2 stub. Two other genomes (*Cirrhinus mrigala*, *Saguinus oedipus*) were excluded from S10 by E5 because their annotations cannot build a gene as long as an ITPR *at all* — *C. mrigala*'s longest annotated gene genome-wide is 42 kb. These are three different systemic annotation pathologies, each of which removes a whole family from the protein databases for that species, and none is visible from the protein side. Worth a paragraph in the audit section and possibly a supplementary panel: how many of the 309 swept genomes have an annotation that *could not* represent this family however good the assembly? | open (S18 / S14a) |
...
| 2026-09-07 | S8 | **S7 asked S8 which side of the vertebrate duplication each cyclostome ITPR lineage attaches to, and flanking-gene synteny cannot answer it.** All 6 loci carry 20 informative flank symbols, so the window is not the limit; the highest overlap any reaches with a gnathostome paralog consensus is 2, and random control windows reach 2. The two species' calls also point at different paralogs. The obstacle is that cross-species flank matching here is by gene *symbol*, and 550 My of rearrangement plus cyclostome annotations that name 27-29 % of coding genes leave no shared vocabulary. Needs a name-independent instrument: Compara/OrthoFinder orthology calls on the flanking proteins, or the 2R paralogon reconstructed from a cyclostome-anchored gene tree. | **closed 2026-09-08 (S13)** — the second of those instruments was built and the question has an answer, though not the one the phrasing anticipated: no cyclostome lineage attaches to *either* side of the vertebrate duplication. The tree makes three hagfish/lamprey orthology pairs; one (100/100) is sister to the ITPR2+ITPR3 clade and the other two form a cyclostome-only clade outside everything, so the loci are three anciently separate lineages rather than copies of ITPR1, ITPR2 or ITPR3. S8's own flank calls were re-joined locus by locus and stay underpowered — 4 of 6 got a call, every one inside its own null — which is exactly what S8 reported. → `results/reconciliation/cyclostome_loci.tsv` |
...
| 2026-09-06 | S6 | **The cyclostome grade carries three ITPR loci per genome and the sweep's ITPR1 bait wins every one of them.** Sea lamprey (LOC116947491 / LOC116951169 / LOC116952798), inshore hagfish and *Myxine glutinosa* each carry three full-length loci; across 19 census records in that band, 6 carry a paralog label, all of them `ITPR1`, all from the S5 bait attribution and none from an annotation. A constant attribution is no information about which locus is which, so S6 enters the whole grade unlabelled. **Whether those three are 1:1 orthologs of ITPR1/2/3 (2R, shared with gnathostomes) or a cyclostome-specific expansion is the single sharpest test of the 2R hypothesis this project can run** — and it needs synteny as well as the tree, because three tips at 60-70 % identity will not separate the hypotheses on sequence alone | **closed 2026-09-08 (S13)** — the answer is **neither**. The six loci are three anciently separate lineages, each pairing one hagfish locus with one lamprey locus, so every duplication among them predates the ~459 Ma hagfish/lamprey split and none is a recent expansion; and none sits inside ITPR1, ITPR2 or ITPR3, so none is a 1:1 ortholog either. What they do settle is the placement: one pair nested sister to ITPR2+ITPR3 puts the ITPR1 / ITPR2+ITPR3 duplication on the **vertebrate stem**, and drop the six tips and it falls back to the gnathostome stem. Synteny could not corroborate it (S8, still underpowered), but the long-branch objection was measured and does not apply — 0.96–1.04× the median root-to-tip distance. → `results/reconciliation/report.md` §7 |
...
| 2026-09-04 | S5a | **Teleost ITPR1 3R duplicates are half-named in RefSeq.** Both *Takifugu rubripes* and *Genypterus blacodes* carry two ITPR1 loci, one annotated `itpr1b` and the other left as an unnamed `LOC` — in *Takifugu*, `LOC101074739`. Two RyR loci in the same genome are unnamed the same way. A name-based census cannot reach these, which is why the sweep carries every locus whose annotation does not already name it correctly into census v4. A concrete, paired worked example for the correction list. **S16 measured the scale, and it is not two species: of the 73 ray-finned genomes carrying exactly two ITPR1 copies, only 20 name both — 34 name one and leave the other a `LOC` or locus tag, and 19 name neither.** The naming is also asymmetric: across the named copies `itpr1b` appears 45 times against `itpr1a` 15, so the databases are systematically better at one 3R co-ortholog than the other. S16's `expansions.tsv` carries every copy with its coordinates, its annotation and its synteny block, which is the correction list in draft. | open (S18) — now quantified |
...
**D51 — A paralogy link is only 2R evidence if it is dated.** Ensembl
Compara puts a duplication node on every paralog pair, and S16 reports the
2R test at three nested vocabularies (`all_levels`, `2R_window`,
`2R_core`). It changes the answer: of the two flanking families S8 found
between the ITPR neighbourhoods, `GRM7 ↔ GRM4` is a *Vertebrata*
duplication and `BHLHE40 ↔ BHLHE41` is *Opisthokonta* — real paralogy whose
two copies happen to sit beside ITPR1 and ITPR2, and not an ohnolog pair.
An undated paralogon test cannot tell them apart.

**D52 — The ryanodine receptors are this project's positive control, not
only its hazard.** D14 exists because RYR1/2/3 carry every ITPR-diagnostic
domain. S16 uses the same fact the other way: they are a three-member
vertebrate family of the same age and the same 2R and 3R candidacy, so
running the identical instrument on them in the same genomes measures what a
true answer scores here. It is what made S16's weak human 2R signal readable
(the control scores the same) and what made the teleost ITPR2/ITPR3
singletons a retention statement rather than a detection failure (the same
genomes kept all six RyRs). Later tasks should reach for the control before
inventing one.

**D53 — A block selected for being the most paralogous to a window cannot
then be tested against it.** S16's quartet test scores every pair of six
windows and their six top blocks; the six window-vs-own-top-block pairs are
significant by construction, because the block was chosen out of ~23,000 for
that property. They are flagged `selection_circular` **in the table**, not
only in the prose, because a reader scanning the file would otherwise read
six results. 0 of the 59 non-circular pairs are enriched.
...
**D39 — A similarity margin is only consulted above the bar that makes
similarity mean anything.** S11's structural family call is D14's test in a
new instrument: best TM-score against the IP3R references, best against the
RyR references, winner takes it if the relative margin clears D7's 10 %.
Gated on the margin alone the rule calls **all three negative controls
ITPR** — a dynein heavy chain, a Cav2.1 and a talin each beat their own
runner-up by about 30 % of their score while scoring 0.17–0.27 against
everything, because a margin between two non-matches is still a margin. The
call therefore requires the winner to clear TM-align's published same-fold
bar (0.50) *before* the margin is read. With the gate, 0 of 3 controls is
called and 20 of 20 callable census-named structures agree with the census.
The general rule: a relative margin is scale-free by construction, which is
exactly why it needs an absolute floor underneath it.

**D40 — A refusal is not a disagreement.** The same instrument declines six
non-vertebrate models. They are recorded as declined and their
`call_agrees` column is left **blank, not 0** — scoring a refusal as a
disagreement would turn "this model is too partial to place" into "shape
contradicts the census". S7's `unconstrained` tips and S23's `uncontrolled`
genomes are the same rule in earlier instruments, and S11 adds the piece
that makes it checkable: the report prints the **arithmetic ceiling** each
declined structure could have reached (its residue count over the
reference's), so "too short to pass" is a claim that can be falsified —
and here it was. Every one of the six had the headroom and fell short on
similarity instead.

**D41 — A failed run is never cached, and a running stage owns a lock.** An
interrupted S11 TM-align run left its Python parent orphaned to init while
its children were killed; it went on spawning work beside the driver that
replaced it, and the SIGKILLed pairs were written to the cache as
zero-score results with empty error text — ten permanent false negatives
that only the stage's own self-test caught. Both halves are now closed: a
result that is not `ok` is not written, so the next run retries it, and the
stage claims a pid-checked lock (checked against a *live* process, so a
crash does not block the next run for ever). S7 recorded this incident for
IQ-TREE; it recurred here in a different tool, which is what makes it a
rule rather than an anecdote.

**D42 — A validation rule must have no tuned threshold, and must be
reported beside the failures it rejects.** S12 validated each reference by
translating it and requiring high identity against the sweep's own protein,
with a 0.95 floor. That floor is not a statement about correctness: a
frameshift costs the reading frame from where it sits, so a **correct**
reference scores 1.00 with no frameshifts and 0.92 with nine, and the floor
was rejecting correct references for carrying frameshifts the sweep had
already recorded. It was replaced by colinear block placement, whose pass
rule is a statement the model makes about itself — *a block may fail to
place only if the model's own frameshift count can explain it* — so there
is no number to tune. The second half is what makes it checkable: every
reference is re-scored against its own broken versions (blocks reversed,
wrong strand) and the three numbers are committed side by side per locus in
`reference_validation.tsv`. A rule reported without the failures it rejects
is asking to be believed. S23's `separation()` and S8's threshold sweep are
the same idea arrived at from the other direction.

**D43 — A ported control's *result* is re-measured and reported, never
inherited; and a risk that can be measured is not argued away in a
caveat.** CLAUDE.md already says port the method, never the result. S12 is
where that stopped being abstract, twice in one task. The PIEZO project's
reversed-CDS decoys collected 0 reads across 78 runs; here they collected
**42 across 67 runs**, in 2 of 469 run x locus comparisons, confined to
~64 bp of one 8,185 nt decoy. It changes no detection call — the decoy is a
per-run floor and the affected locus clears it in the same run — but
writing "zero" because zero was the ported answer is exactly the failure
the rule exists to prevent, and the prior therefore renders `contradicted`
with the number printed. Separately, PIEZO's S12 dismissed cross-mapping
between paralogs in a caveat ("well under 70 % nucleotide identity"); the
ITPR paralogs are more similar than the PIEZO ones, so S12 tiled every
reference exhaustively with synthetic reads and mapped them back instead —
**0 of 12,500 assigned elsewhere**, committed in `crossmap_control.tsv`. An
argument in a caveat and a measurement in a table are not the same evidence,
and the second costs minutes.

**D44 — A reconciliation rule validated on binary trees is not validated on
the tree you collapsed.** The classic LCA duplication test — a node is a
duplication when its species mapping equals a child's — is correct only on a
binary gene tree. S13's support-collapsed variant makes the root of the
vertebrate subtree a four-way polytomy whose children map to Cyclostomata,
Gnathostomata, Cyclostomata and Gnathostomata: no child maps to Vertebrata, so
the binary rule calls it a **speciation** and the whole collapse would have
been reported as "the duplications disappear when the unsupported nodes go" —
a headline-reversing artefact of the rule, not a result. S13 uses the standard
non-binary generalisation (Vernot et al. 2008) and `s13_test_recon.py` T2/T3
are its positive and negative halves. The general rule: when a step changes the
*shape* of an input (collapsing, pruning, merging), re-check that every
downstream rule was defined for the new shape.

**D45 — An absence in a paralog cell is not an absence of a gene when the
genome carries family loci the instrument cannot assign.** S13's loss audit
first returned four corroborated losses — ITPR2 and ITPR3 in *Myxine
glutinosa* and *Petromyzon marinus* — and both genomes carry **three ITPR loci
apiece**, all filed by S5 in the ITPR1 cell because the bait panel has no
cyclostome-labelled bait to offer the other two (S5's six unfilled slots
include all three paralogs in cyclostomes). The ledger's `absent` there is a
statement about a *cell*, and S7 says the same thing from the other side: every
cyclostome tip sits outside all three paralog clades. The verdict
`paralog_unassignable` is therefore a positive test on two independently
committed numbers — the genome's spare locus count and the species'
...
S18 caches the four-minute pass over 274 gzipped GFF3 files, and the first
version cached the finished audit rows — measurements *and* verdicts. When
`name_verdict` later gained `paralog_unspecified`, the committed table still
carried the old labels and mouse *Itpr1* read as the annotation naming a
different paralog, with no error anywhere. Every state and every verdict is
now recomputed on load (`s18_locus_audit.apply_verdicts`), and the cache
carries a SHA-256 of the two reader modules so a change to the *parsers*
invalidates it rather than being served silently. The general form: a cached
artefact must be invalidated by every input that can change it, and the
cheapest way to guarantee that is to cache only the expensive, rule-free part.

**D56 — A threshold this project already has is not re-derived by the task
that inherits it; it is validated.** S18 needed a bar for "one annotated model
delivers this gene" and `s5_classify.ANNOT_CDS_FRAC` (0.50) already is one,
read by S5, S10 and S23. Choosing a second here would fork what the project
means by an annotated gene, so the calibration was spent on the open question
instead — *where does 0.50 sit in the distribution it is applied to?* — and the
answer is a result: over 1,077 correctly-named, fully-recovered loci a single
model covers a median 0.993, so the inherited bar is that distribution's 3.6 %
point and is conservative by construction. This is `s5_calibrate_margin.py`'s
shape (asking whether an inherited constant transfers) applied to a constant
that turned out to transfer. The rule is not "never measure a threshold" — it
is that a second definition of the same thing needs a stated reason, and
"my task would prefer its own" is not one.

**D54 — A coordinate carried from one protein to another must arrive on an
anchor that the alignment does not know about, and an element no annotation
carries must be located by measurement rather than drawn.** S17 needed the
selectivity filter, the gate and the ten IP₃ contacts in ITPR1 and ITPR2
numbering; all three were measured on 6DQN, which is ITPR3. A pairwise
alignment will always return *a* coordinate, so the transfer is only evidence
if it can fail: the filter must arrive on the same GGGVGD motif, the gate on
the same two lining residues, and a failed anchor aborts rather than writing a
plausible number (`s17_test_constraint.py` T5 mutates the motif and requires
the rejection). The second half of the rule is what made §5 an answer. The
Pfam channel domain read as *less* constrained than the receptor's own
linkers — a strange thing to report about the pore of an ion channel — and the
cause was a 50-residue luminal loop inside it that no annotation names. A
boundary drawn on the conservation profile would have been the profile
explaining itself, so the loop was defined instead as the contiguous run of
channel residues whose Cα lies beyond the membrane on the luminal side of the
axial span S0 measured on the same structure. That derivation knows nothing
about any column, and it landed on the dip. Corollary: **the two things a
carried element needs are an anchor and a provenance column**, and
`domain_map.tsv` carries `kind`, `transferred_from`, `check` and `anchor` on
every row precisely so a reader can tell a measured boundary from a
transferred one.

**D48 — A sensitivity analysis reports the *map of fragility*, and it keeps
the axis that cannot move the answer.** S15b's count is zero, so the number
carries no information and the grid around it carries all of it. Two rules
come out of building that grid. First, an axis the result provably cannot
depend on is **carried as a column and reported as invariant**, not dropped:
Dollo parsimony counts edges and cannot read a branch length, and a matrix
that quietly omitted the branch-length axis would be indistinguishable from
one that had tested it and found nothing. Second, the grid is what turns a
judgement into a measurement. D46 chose family-level presence as the primary
coding on the strength of the `co_trace` population; the grid *measures* that
choice — the family coding manufactures a loss in 2 of 32 settings and the
paralog-resolved one in 18, up to 45 loss edges. Any later task reporting a
threshold-dependent result states which settings manufacture it, and walks
the threshold across its own measured uncertainty **before** any invented
value: moving S15a's reconstruction bar across the whole gap its calibration
found (0.100–0.174) changes no cell in 927.

**D49 — A within-genome paired test makes a cell and its siblings one
observation, and strata from it are not independent corroboration.** S15b
stratified D47's identity-matched paired sign test by vertebrate class and
found the ITPR3 indel excess is a bird result (25 genomes to 2, q = 4.5e-5,
against 7 to 6 in Actinopteri). The same table shows ITPR1 and ITPR2 in Aves
carrying significant *deficits* — which is not three findings but one, because
each cell is scored against its own genome's other family loci and an elevated
ITPR3 makes its siblings look deficient by construction. `sibling_of` is
written into `lesion_class_controls.tsv` for exactly that reason. The second
half of the rule is the control that has to be printed beside it: 21 of the 27
bird pairs are in assemblies **below D4's contiguity bar**, which is where the
test has its power, and the 6 above it all point the same way without being
enough to test. So the verdict is `underpowered` and the next instrument is a
different *sample* — birds at chromosome level — not a different statistic.

**D57 — A search's sensitivity is measurable, not estimable, when the family
has no losses; and the control must be able to fail.** S19's whole design
rests on S15b's zero: all 923 assignable genome × paralog cells hold a gene
established by evidence the sweep's locus placement did not produce, so every
ledger cell that is not `found` is a **false negative of the method**. The
rate is 140/923 (15.2 %). Three rules make that a measurement rather than a
restatement. (a) The four `paralog_unassignable` cyclostome cells are in
**neither** the numerator nor the denominator — S15a declined to call them
present or absent and re-deciding here would be a fork. (b) The **RyR sister
cell is carried as an independent replicate**: present in every vertebrate,
swept by the same aligner in the same assemblies, and using none of S15a's
state assignments. It gives 42/309 (13.6 %), indistinguishable from the ITPR
series (Fisher *p* = 0.58), which is what stops the number being read as the
search agreeing with itself. (c) A false negative must be **constructible** —
`s19_test_methods.py` T5 builds a cell that is demonstrably present and
demonstrably not found and requires it counted, because a control that can
only ever return zero misses is not a measurement.

**D58 — D4's contiguity bar was chosen a priori and is now calibrated;
it stands.** The bar (142,212 bp of contig N50, the median measured ITPR
genomic span) was derived from gene geometry with no error rate in it. Scored
against S19's measured false-negative rate it gives **0.9 %** residual error
on the ITPR series and **0.0 %** on the RyR sister, retaining 189 of 309
genomes. It is conservative against the conventional 5 % target, which the
floor scan reaches at 100 kb, and about right against 1 %. The bar is
therefore kept rather than replaced — and the cost is reported beside it,
because a floor removes 38.8 % of the scope and removes it
disproportionately from the clades a loss survey most wants: the margin
species S4 added for their *proteome* gaps are largely the same genomes whose
assemblies cannot carry the gene.

**D59 — The kill rule written to catch iterative-search drift measures the
wrong axis, and the replacement is proposed rather than applied.** Scored as
classifiers of a drift outcome measured on the *finished model* (the share of
a run's final included set that neither profile scores — the seven runs
separate at 0.34 against 0.81 with nothing between), D10's rules give: **K1
sensitivity 0.00**, K2 0.33, K3 sensitivity 1.00 at specificity 0.25. K1 fires
on none of the seven runs including all three that drifted, because
off-family accretion *dilutes* the sister-family share (D10b, now measured:
the share **fell** over the run in 4 of 7). Moving K1's own 0.10 threshold to
the **off-family** share separates all seven (sensitivity 1.00, specificity
1.00), firing at round 2 or 3 in exactly the three that drifted. It is
**not applied**: seven runs with an inherited threshold cannot establish a
specificity of 1.00, and applying it would overturn committed verdicts, which
belongs to a session that owns those tables.

**D60 — A self-test must not be able to damage the artefact it tests.** S19's
first build called the real `criterion_trace` on constructed rounds, which
wrote its two synthetic rows over the committed `kill_criterion_trace.tsv`;
the report then read 2 jackhmmer runs where there are 7, and nothing failed.
Two rules follow. Any routine a negative control exercises takes a `write`
flag and the control passes `write=False`. And the suite itself checks the
SHA-256 of every committed table before and after it runs (`T15`), so the
class of bug cannot recur silently. The suite also runs **twice** — before
any stage writes, where the four artefact-dependent checks report as
*skipped* rather than crashing on a clean tree, and again from the `tables`
stage once those tables exist, with the result recorded in
`methods_stats.json`.

**D61 — A manuscript's citations are keys, resolved at build time; its
bibliography is rendered, never typed.** The section files — the only
hand-written files in the package — cite with this project's stable `[R22]`
keys against `results/s0_baseline/references.tsv`, the same table the
literature review is built from. Both the stitch stage and the PDF stage
renumber them into order of first appearance and render the reference list
from that table, so no author, title or year is ever retyped into a section
file, a section can be reordered without renumbering anything, and a cited
key with no reference row is a build error rather than a dangling number.
This is `s0_review_build.py`'s rule applied to the manuscript; the reason it
is a decision rather than a convenience is that a bibliography typed beside
the text is the one part of a paper no table check can reach.

**D62 — A typeset build is not finished until its own log has been read for
missing glyphs.** The PDF font (TeX Gyre Termes) was chosen because Latin
Modern silently drops ω, χ and ≥. It has no **subscript** glyphs either, so
`IP₃` — the family's own name, in the title — reached the first build as
`IP`, on a page that otherwise looked correct, with nothing but a
`Missing character` warning in a log the build did not read. Two rules
follow. The converter handles sub- and superscript runs **unconditionally**
rather than only after a word character, because after a previous
substitution the preceding character is a bracket and the run is left behind
(`\(\omega\)₂` was the case that showed it). And anything spliced in after
the per-section conversion — the generated bibliography, the YAML title —
goes through the same converter. The build is checked by grepping its own
pandoc log for `Missing character`, which is now empty.

**D63 — A supplementary figure exists so a reader can check a join, so the
join is checked in code first, and the check is exhaustive.** The trimmed
alignment and the untrimmed one are two coordinate systems that every
residue-level claim in the paper crosses; trimAl writes no map between them.
S6 recovered one with `trimal -colnumbering` and committed it, and S24 does
not trust it: every trimmed column is compared against the input column the
map names, over every sequence, and a disagreement raises. Sampling would
not do. An off-by-one map still maps every column to a column, still parses,
and still produces a figure — it would simply renumber every domain track,
every variant and every conservation value by one, and nothing else in the
project would notice. The same rule covers the residue joins: every
clinically labelled position is checked against the residue its own
paralogue's per-residue table holds there, and every aligned partner against
the residue the *other* paralogue's table holds.

**D64 — A structure may carry a position from another numbering only if it
passes an identity test, and the refusal is the result.** A ClinVar position
is a number in human canonical numbering; a deposition numbers its own
construct, and an AlphaFold DB record numbers whichever isoform the API
served. So a structure earns the right to carry human variant positions only
when *every* residue it shares with the human per-residue table carries the
same amino acid. Human ITPR2 (9YKK) and ITPR3 (8TKG) pass at 2,168/2,168 and
2,210/2,210. The ITPR1 cryo-EM reference is a rat structure (736/2,300) and
AlphaFold DB's human ITPR1 model is the 2,695-residue Q14643-4 isoform
(479/2,695) — the same isoform trap S11 measured, arriving in a second place
— so ITPR1's 55 pathogenic positions are not drawn at all. Drawing them on
rat coordinates would have produced a figure indistinguishable from a
correct one.

**D65 — A figure is not reproducible until its file is.** matplotlib stamps
the wall clock into a PDF's `/CreationDate`, so every figure in this project
differed from its own rebuild by two bytes and no SHA-256 recorded against a
figure pdf meant anything — including the ones in `align_stats.json`,
`structures_stats.json` and every other stats file that records one.
`figstyle.save` now drops the field, which changes nothing that is drawn.
The self-test that guards it does **not** rest on byte-identity alone: two
...
**D69 — A comparison between two regions of one protein must declare its
boundaries, because on this receptor the answer turns on one of them.** S22
asks whether the IP₃-binding core is more constrained than the pore. Drawn
on Pfam spans — the way the comparison would be made by default — the core
wins in all three paralogues at q < 1e-37. Remove the fifty residues of
luminal loop that S17 located by geometry inside PF00520, the least
conserved sequence in the whole receptor, and the *pore* wins in ITPR1 and
ITPR3 and the comparison is flat in ITPR2. Both are correct about their own
region and neither is correct about "the pore". So S22 defines both modules
twice — a primary definition derived from a measurement this project made,
and the field's own definition beside it — checks each against something it
does not contain (the core must hold all ten measured contacts and no pore
residue; the pore both filter and both gate residues and no contact) and
raises rather than writing a region that fails. Every test is run under all
four combinations and the reversal is reported, not resolved.

**D70 — A functional site is measured as a distance, not as a label.** A
contact/not-contact flag on ten residues discards the one thing a structure
can say that an alignment cannot. S22 measures every residue within 15 Å of
IP₃, all-atom rather than Cα (a Cα trace puts an arginine 8 Å from a
phosphate its guanidinium is bonded to), in six independent IP₃-bound
depositions rather than one. Three things follow. The reader must recover
S0's ten contacts in S0's own structure or the stage raises — the positive
control is a hard failure. The consensus contact set is **twelve**, because
Ala276 and Arg411 sit inside 4.5 Å in a majority of the depositions and
outside it in 6DQN. And the result the label could not reach: the ten
contacts beat the rest of the binding core and beat the rest of the pocket
in none of the three paralogues, so what selection holds is a neighbourhood
and not a contact set.

**D71 — A lineage contrast between clades is a clade contrast until it is
matched on divergence.** S22's PLC-absent taxa are oomycetes and
early-diverging fungi, sitting at median pore identity 0.358 to the human
reference against 0.589 for the rest, and the paired core-minus-pore
statistic is itself correlated with divergence. Pooled, they look relaxed at
p = 9.7e-6; matched to PLC-present records within 0.03 pore identity — S15a's
identity-matched paired design (D47) applied to the same kind of claim — the
effect is gone (median −0.0064, 95 % CI −0.016 to +0.015, p = 0.87). The
verdict is a **bounded null** and not an underpowered one only because the
positive control is measured on the same instrument at the same divergence:
...
committed table it was testing. All 18 fire; `thesis/guard_check.tsv` records
what each said.
...
**D76 — In a scientific document a heading names its subject and a sentence
carries a predicate; the em-dash is not a substitute for either.** S25's first
draft was written in a register that reads well in a narrative and badly in a
thesis: headings that were fragments and gave no context on their own (*The
receptor, and the question*; *The problem stated precisely*; *The claim that
failed, and what it turned into*), openings that withheld their subject until
the second sentence, and 464 em-dashes across 27 chapter files, many of them
carrying the clause a reader needed. The user read it and said so, and the
whole document was rewritten: every heading now states what its section is
about without needing the one above it, every section opens by naming its
subject, and there are **zero em-dashes in the chapter sources**, with each
one replaced by the punctuation or the sentence break the sense actually
wanted. Word count rose from 58,436 to 60,018, which is what saying a thing
plainly costs. Three general rules come out of it and apply to S26's papers as
much as to the thesis. A heading is read out of context, from a table of
contents or a search result, so it has to be legible there. A section's first
sentence is the one most often read alone, so it may not begin with a pronoun
or a definite article pointing at something the reader has not been given. And
an em-dash that carries a load-bearing clause is a sentence that has not been
finished; where the punctuation is doing real work, the sentence should be
split. The rewrite passed every build guard unchanged, which is the one thing
worth noting about it methodologically: 220 claims, 103 figure placements and
80 citations survived a full-prose rewrite because none of them lives in the
prose.

**D77 — The thesis is authored by Claude, and the title is a complete noun
phrase with nothing appended to it.** Two corrections the user made after
reading the rewritten draft, both of which are rules rather than one-off
edits. The author line reads *Claude (Opus 5, Anthropic)* with the
correspondent named separately as the point of contact, because the author has
no address of its own and a document should say who wrote it. And the title
went from *A genome-scale census of the inositol 1,4,5-trisphosphate receptor
family, and the methods built to make it* to the same phrase without the
appended half, with the scope moved into a subtitle. The appended clause was
the same fragment pattern D76 removed from the headings, surviving in the one
place a reader sees first. **The general rule: a title is a heading, and every
rule about headings applies to it.** The author and title now live in
`s25_lib.AUTHOR` and `s25_lib.TITLE` and are read by the PDF stage rather than
typed into it, so the front matter and the PDF metadata cannot disagree.

**D78 — A style rule is only applied when it is checked mechanically.** D76's
first pass fixed the headings that were obviously confusing and removed every
em-dash, and the user's follow-up found that it had not gone far enough. A
systematic check then found 30 section headings with no finite verb, 15 bold
paragraph lead-ins that were fragments, five appendix class labels that were
noun phrases with counts attached, and one passage that listed five findings
as verbless fragments. The lesson is the project's own rule about negatives
applied to prose: a style pass that has not been checked mechanically is a
style pass that fixed the instances somebody happened to notice. The three
checks now used, and reusable by S26, are to parse every heading for a finite
verb, to parse every bold paragraph lead-in the same way, and to count dashes
by type so that a legitimate en-dash range is not confused with a
parenthetical em-dash.

**D79 — How the work was carried out is part of the record, and its numbers
are measured from the repository rather than recalled.** The user asked for a
chapter describing how Claude Code carried out the project. The obvious way to
write it is from memory, and that is the one way this project does not write
anything. `s25_production.py` therefore derives every figure the chapter
quotes from files under version control: sessions and days from the session
log, tasks and completions from the ledger, decisions from the decisions log,
scripts and lines from the source trees, tables and figures from
`git ls-files`, report words from the rendered reports, and commits and their
co-author trailers from `git log`. Twelve of those numbers are declared in the
claims ledger, so a sentence about the project's scale fails the build when the
scale changes, in exactly the way a sentence about the data fails when the data
changes. Two consequences worth stating. The chapter is honest about the
division of labour, including what the human collaborator contributed and
what he corrected, because a description of an autonomous agent's work that
omits its corrections is not a description. And the chapter's own closing
section is about the failure modes of the arrangement rather than its
strengths: a fluent agent produces plausible prose about work it has not
checked, a rule that fires on nothing looks exactly like a rule that cannot
fire, and a style pass that has not been checked mechanically only fixes what
somebody noticed. All three are things this project did, caught and recorded.

**D80 — A figure legend says what the figure shows and why that matters, and
a "what is new" section carries the literature it is new against.** The user
read the assembled thesis and asked for two things that turn out to be the
same rule seen from two ends. Every one of the 103 figure legends was written
to describe the figure and to defend its construction, which is what a legend
in this project's per-task reports does, and none of them told a reader why
the panel was worth drawing. And §15.1 stated six new results in a page and a
half with no citations at all, so a reader had no way to tell which of them
contradicted a standing position and which merely filled a gap. Both are
fixed: every legend now closes with what the figure changes for a reader, and
§15.1 states each result with the literature it bears on, which took the
section from 409 to 1,893 words and the new-reference count from 58 to 76.
Three general rules come out of it. **A legend that only defends its own
construction is written for a reviewer and not for a reader**, and the
importance sentence is the part a reader actually needs, so it goes last where
it is read last. **A claim of novelty is a claim about a literature**, so a
section headed "what is now known that was not" has to cite what was known,
and this one cites eighteen references it did not previously need. And the
audit rule scales without change: those eighteen were declared by identifier,
resolved live, and all eighteen passed on the first attempt, which is what a
rule looks like once the failure mode it was written for has been removed.

**D81 — An appended sentence is not an edit; a paragraph that says a thing
twice says it once badly.** D80 asked every figure legend to explain why its
figure matters, and the fastest way to satisfy that is to append a sentence to
each legend. Doing so produced 50 legends of 103 that stated the same point
twice, either inside the legend or against the paragraph immediately beside
it, and the duplication was invisible while writing because each half read
well on its own. Two mechanical checks now exist and both are cheap: compare
every pair of sentences inside a legend for shared content words, and compare
every legend against the two paragraphs either side of it for shared word
runs. Both were run to zero. **The general rule: a request to add something to
every instance of a thing is a request to rewrite each one, and the check for
whether that happened is a repetition detector rather than a reading.** The
same applies to S26's papers, where a legend has less room to waste than a
thesis chapter does.

**D82 — A number typed into a report generator is the one number a
report-generation rule cannot protect.** D13 says a report is rendered from
its committed tables, and the annotation audit's generator satisfied that for
every number except one: the count of vertebrate reference proteomes was typed
as 764, which is the line count of the sweep's manifest including its header.
The correct figure, 763, is in the sweep's own statistics file and in three
other chapters, so the package disagreed with itself by one for four editions
and no check could see it, because the claims ledger verifies what a document
says against a table and not what a *generator* says against its own source.
The generator now reads the number. **The general rule: grep every report
generator for integer literals, because a rendered report's credibility comes
from having no typed numbers at all, and one is enough to break it.** The
audit that found this was reading prose, not running a check, which is the
argument for D83.

**D83 — Reading every sentence is a distinct pass, and it finds a class of
defect no build guard can.** The user asked for the thesis to be read sentence
by sentence, in context, for factual correctness, grammar, meaning and use to
the reader. Every guard in this project passed before that pass began and
passed unchanged after it, and the pass still found a typed number that
contradicted three chapters, four counts that had gone stale because the
project grew past them, two cross-references pointing at the section before
the answer, six results chapters with no citations at all, and fifty legends
that repeated themselves. **None of those is reachable by a checker**, because
each is a statement that is well formed, internally consistent and wrong or
useless in context. Three of the five classes were then converted into checks
so they cannot recur: two new claims pin the decision and session counts to
the measured table, and the two repetition detectors are committed. The other
two, wrong cross-references and missing citations, remain a reading problem.
The lesson is the one this project keeps relearning in a new place: a check
tells you a document is consistent with its data, and only a reader tells you
it is worth reading.

**D84 — A figure that opens a document has to show the object, not a
property of it.** The thesis's Figure 1.1 was `signal_hierarchy`, three panels
of calcium event traces, and its legend described it as the receptor's place
in the phosphoinositide pathway, which is not what it draws. The citation in
the text was for the pathway too, so the first figure a reader met answered a
question nobody had asked and the legend answered one the figure could not.
That is the failure D80's legend rule was written for, arriving in the one
place a reader cannot skip. The replacement, `receptor_overview`, shows the
object at the three scales the rest of the document uses: the pathway that
makes the ligand as a schematic, the channel at the dimensions measured on
6DQN with its linear domain architecture beneath, and the residues themselves
at the two positions the structure marks. `signal_hierarchy` moves to §1.3,
where puffs and waves are actually discussed, with a legend that describes its
own panels. **Two general rules come out of it.** A figure's panels may have
different provenance, and when they do the tag belongs on each panel rather
than one corner tag covering all three, because "schematic" over a measured
panel understates it and "measured" over a drawn one is false. And an
orientation figure earns its place by being complementary to the figures that
follow rather than a summary of them: this one names, in its own legend, which
later figure carries the measured version of each panel, so a reader knows
what is coming rather than seeing the same thing twice.

**D85 — A check that passes is a claim about what it looks at, and the page
is the only place a layout defect exists.** S28 ended its first build with
every mechanical rule green: zero prose failures, every figure passing the
text-overlap check, a clean PDF log. The page-by-page read then found figures
printing out of numerical order in six chapters, panel titles hidden behind
the panel beside them, legends printed over their own data, stray rules in the
middle of chapters, half-empty pages ahead of pinned figures, and 97 figures
11.9 pt into the margin. The last had been true since the project was set up:
`s14_lib.W_FULL` draws every figure for a 2.0 cm margin and both PDF stages
set 2.2 cm, and nothing read the log for it until S28's overfull check did.
None of these were visible to a check that was passing, because each check
looked at something else: the overlap test compares text with text, the
numbering was right by the figure map's order, and the log check did not
exist. **Two rules follow.** Every class of defect a page read finds becomes
a check where one can be written (here three: a title running into another
panel, a title crowding the next panel's letter, a legend over its data, each
with a guard case), and the read stays a step of every typesetting task
rather than being retired once the checks exist. And a number a reader sees
follows the page, never a declaration order: figure numbers are now read off
the chapter sources, so a chapter can be reordered without a figure map edit
and cannot print 6.5 before 6.1.

**D86 — A paper series is enforced by its dependencies, not by its table of
contents.** S26 split the work into six papers. The risk the brief named was
a set of slices that only make sense read together, and the test for it is
P6: what each paper claims if none of the others is published. Written as
prose, P6 is a paragraph anybody can write about anything. Enforced, it
becomes a statement about data ownership: each standalone answer names the
ledger claims it rests on, and each of those claims must come from a table
primary in the same paper. Likewise P2 (a paper's controls live in its own
results) and P5 (a paper places only figures from its own results; to show a
sibling's result it cites the sibling). Three rules follow for any document
set built from one body of results. **An assignment table that a document
can silently deviate from is decoration**, so both this series and the thesis
now fail on an undeclared difference between their two tables. **A directory
is split only where a control forces it** (P5s): S19 measured four things for
four papers, and keeping it whole would have left three papers' central
controls living in a sibling. **Numbers shared between documents are
declared once**, carried by identifier across the manuscript, thesis and
paper ledgers. The measured result is that the six papers share only four of
702 claims. The series is a set of distinct papers, not one paper quoted six
times, and that is what P6 was for.

**D87 — Writing a result a second time is an audit of the first writing.**
Six papers were written from the same committed tables as the manuscript and
the thesis, by writers told to trust the tables over the earlier text. They
found three defects the earlier documents had carried for several editions,
all of which the existing guards passed. The deep constraint layer was "the
second-best of four" in the S17 report, the manuscript and the thesis, while
the table printed beside the sentence ranked it third. The word was typed
into the report generator, which is D82's failure, the one number a
report-generation rule cannot protect. The manuscript's headline of 940 of
1,232 genes unreachable from protein databases described the IP₃ receptor
family, but the count included the 309 ryanodine receptor control genes, and
the family's own figure is 744 of 923. And S26's own first build printed
Extended Data figures out of numerical order: D85's defect, recreated
because legend order followed the typing, not the numbering. **Two rules.**
An ordinal or a rank in a generated report is computed, like a count
(`s17_report_variants.py` now reads the rank off the table). And an
