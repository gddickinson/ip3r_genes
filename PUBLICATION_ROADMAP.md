...
> **Next session: S23.** S20a and S20b are both complete. S23 takes S20a's absences to assembly level — a proteome absence is an annotation fact, only an assembly search makes it a biological one. Priority list from S20a: **Streptophyta 0/384**, **Dikarya 0/1,353**, **Apicomplexa 0/60**, the patchy fungal losses (Glomeromycota, Mortierellomycota, Kickxellomycota, Microsporidia) and the *Cymbomonas* copy-number question (12 of the 22 complete green-algal genes are in one species). Read `results/s20_sweep/report.md` first: it also carries the four D10 outcomes and the two sensitivity effects `s20_test_sensitivity.py` found.
> row now that S0–S5 are complete. Read `results/genome_ledger/report.md`
> first. Three things it establishes that S20 inherits: **D14 never once
> failed at genome scale** (2,144 loci, no contested locus), so a genomic
> sweep separates ITPR from RyR where a protein search does not;
> **`MIN_LOCUS_IDENTITY` = 0.40**, without which a large `-G` chains
> shared-module hits into phantom loci in big genomes; and **D4's
> contiguity bar disqualifies 120 of 309 vertebrate genomes from any
> absence claim** — S20's invertebrate and protist assemblies are on
> average worse, so that bar has to be applied there before anything is
> called absent.
>
...
| S23b | **The full non-vertebrate sweep** → per-genome copy-number ledger + the plant/fungal/apicomplexan absence claims at assembly level → census v6 | S23a | in_progress | S20a sharpens the target list: Streptophyta (0/384) and Dikarya (0/1,353) are the headline absences to take to assembly level, plus **Apicomplexa 0/60**, the patchy fungal losses (Glomeromycota, Mortierellomycota, Kickxellomycota, Microsporidia) and the *Cymbomonas* copy-number question. |
...
| S7 | ML phylogeny: IQ-TREE 2 + ModelFinder + 1000 UFBoot, rooted on RyR; resolve which two of ITPR1/2/3 are sisters (AU test on the three rooted topologies) | S6, S23b | pending | |
...
| S14a | **Manuscript assembly** — draft, figures, methods, deposit manifest, reviewer self-audit | S3, S5b, S7, S8, S9, S10, S11, S20, S23b, S15–S19 | pending | |
