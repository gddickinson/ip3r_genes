...
> **Next session: S6.** S23 is complete: the family's range, copy number
> and absence claims are settled at assembly level across 194 non-vertebrate
> genomes, and census v6 is the input S6 selects representatives from. Read
> `results/s23_scope/report.md` first — three things it establishes that S6
> and S7 inherit: **copy number outside the vertebrates ranges 0 to 18**, so
> a representative rule that assumes one gene per species is wrong;
> **identity to the nearest bait is not a usable statistic outside the
> vertebrates** (D27/D29), so any selection or filtering rule stated in
> identity has to be re-derived; and **183 new genomic gene models** are in
> census v6, 100 of them annotated but unnamed.
