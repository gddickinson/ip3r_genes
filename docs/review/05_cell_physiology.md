...
as the elementary building blocks of the IP<sub>3</sub>-evoked signal [R08].
...
propagating waves [R07]. Exocrine cells display the local-to-global transition
...
[R89], and oscillation frequency scales with agonist concentration
[R90]. Two classes of model followed — one in which the oscillator is built
from Ca<sup>2+</sup>-induced Ca<sup>2+</sup> release between two pools [R87],
and one in which the bell-shaped Ca<sup>2+</sup> dependence of a single pool of
IP<sub>3</sub> receptors suffices [R88]. The second, built directly on the
...
licensed subset [R92]. This resolves a long-standing discrepancy between the
...
Ca<sup>2+</sup> response [R09]; the linkage is a defined structural apposition
with a measurable gap and tethering elements [R10]; and the resulting transfer
sets the threshold for apoptosis [R11]. Tethering machinery has been identified
in yeast [R94] and in mammalian neurons [R96], and the interface is a signalling
...
they support and in the efficiency of local Ca<sup>2+</sup> transfer [R12].
...
Ca<sup>2+</sup> overload and metabolic dysfunction in obesity [R95] — and the
interface is remodelled in Alzheimer disease [R84].
...
apoptosis [R98], and increased type 3 receptor expression mediates lymphocyte
apoptosis [R97]. The Bcl-2 axis (§4.2) supplies the regulatory logic, and the
...
overexpression is repeatedly reported in cancers [R124, R125].
...
Ca<sup>2+</sup> selectively to lysosomes [R93], placing the receptor upstream of
