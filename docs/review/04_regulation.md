...
and it *decreased* Ca<sup>2+</sup> release in brain preparations [R17];
protein kinase C and Ca<sup>2+</sup>/calmodulin-dependent kinase were shown to
phosphorylate the receptor shortly afterwards [R71], and cGMP-dependent kinase
adds a further site [R72]. The functional sign of PKA phosphorylation is
...
glutamate receptor [R91] is an early and clean demonstration that oscillation
...
IP<sub>3</sub>-evoked ER Ca<sup>2+</sup> release [R18], acting through its BH4
domain on the central modulatory region [R73]; Mcl-1 and Bcl-2 modulate the
channel in ways that protect against apoptosis [R76]; and the BH4 domains of
Bcl-2 and Bcl-X<sub>L</sub> are not functionally equivalent at this target
[R75]. Competition experiments indicate that Bcl-2 and IP<sub>3</sub> can
compete for the ligand-binding domain itself, modulating signalling output
[R77], and dual BH3-like domains have been proposed to underlie biphasic
regulation of gating [R68]. The area is reviewed comprehensively [R78].
...
Ca<sup>2+</sup> signalling [R74], and constitutive IP<sub>3</sub> signalling is
what makes B-cell malignancies sensitive to the BIRD-2 disruptor [R132]. The
...
the compartment it drains — and it does so subtype-specifically [R20]. This is
...
IP<sub>3</sub> rises [R19]; mechanistically it suppresses receptor activity by
competing with IP<sub>3</sub> for the same site [R79]. It therefore sets a
...
IP<sub>3</sub> in some conditions [R80], and calmodulin modulates the cardiac
type 2 receptor directly [R81]. Together with the intrinsic Ca<sup>2+</sup>
...
triggers ubiquitination and down-regulation [R82], and an early report
...
receptor, causes autosomal dominant sensory ataxia [R83]. That a *disposal*
...
[R84]. This positions the receptor downstream of a major neurodegenerative
locus, and is part of the wider argument that ER Ca<sup>2+</sup> handling is a
convergence point in neurodegeneration [R122].
