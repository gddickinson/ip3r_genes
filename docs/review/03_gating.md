...
The structure of the IP<sub>3</sub>-binding core with ligand bound [R05] showed
...
The N-terminal suppressor domain, solved separately [R06], reduces
...
variant in the suppressor domain causes cerebellar ataxia [R120], and the
Gillespie-syndrome variants cluster in this same N-terminal region and in the
channel domain (§9.1).

### 3.2 Ca<sup>2+</sup> as obligatory co-agonist

The bell-shaped Ca<sup>2+</sup> dependence [R13] is the receptor's signature
...
from spontaneous release [R14]; spontaneous release does occur when the
safeguard is stressed [R69], and even quiescent channels show low-probability
openings [R66]. Systematic analysis of gating in permeabilised preparations
established the co-agonist model as an allosteric scheme rather than a
description [R65]; the state of the activation problem before the structures
arrived is reviewed in [R137].

The structural counterpart arrived with the human IP<sub>3</sub>R3 structures
solved with Ca<sup>2+</sup> and IP<sub>3</sub> [R24] and was then tested
functionally: of the candidate Ca<sup>2+</sup>-binding sites visible in the
maps, mutagenesis identified which are actually required for activation [R61].
...
requires IP<sub>3</sub> occupancy at **all four** subunits [R15]. This single
...
without being a permissive requirement [R16], and the effect differs between
paralogues — the type 3 receptor's ATP regulation was characterised separately
and is not identical to the type 1 [R70]. Because the concentrations involved
