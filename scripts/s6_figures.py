...
def fig_identity_heatmap() -> Path:
    ...
    ax.legend([Patch(facecolor=fs.GROUP[g]) for g in groups],
              [fs.GROUP_LABEL[g] for g in groups], loc="upper right",
              fontsize=6, framealpha=0.92, handlelength=1.1)
...
def fig_conservation() -> Path:
    ...
    for label, c0, c1, cls in doms:
        dax.barh(0, c1 - c0 + 1, left=c0 + 1, height=0.55,
...
def fig_coverage() -> Path:
    ...
    for i, g in enumerate(groups):
        ...
        xs = [i + (j - (len(vals) - 1) / 2) * (0.62 / max(len(vals) - 1, 1))
    ...
    ax.set_xticklabels([fs.GROUP_LABEL[g].replace(", ", ",\n")
                        for g in groups], fontsize=6)
...
def fig_group_identity() -> Path:
    ...
    ax.set_xticklabels([f"{a[-1]} × {b[-1]}" for a, b in pairs])
    ...
    short = {"vertebrate_basal": "vert. basal", "invert_metazoa": "inverts",
             "RYR": "RyR"}
    ticks = [short.get(g, fs.GROUP_LABEL[g]) for g in order]
