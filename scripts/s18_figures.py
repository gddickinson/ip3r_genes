...
def fig1_by_source():
    ...
    for ax, scope, letter in zip(axes, scopes, "ab"):
        ...
        title = ("every scorable locus enters the contrast" if letter == "a"
                 else "the same contrast is drawn above D4's contiguity bar")
...
def fig2_calibration():
    ...
    ax.set_xlim(0, 1)
    F.panel(ax, "a", f"the bar sits inside the {len(vals)} loci the annotation names correctly")
    F.despine(ax)
    ...
    ax.set_ylim(0, 1)
    F.panel(ax, "b", "moving the bar re-counts every state")
    F.despine(ax)
    fig.tight_layout()
...
def fig3_family_vs_control():
    ...
    F.panel(ax, "a", "the three paralogs stand beside the sister-family control")
...
def fig4_protein_side():
    ...
    F.panel(ax, "a", "databases name records by symbol or product")
    F.despine(ax, keep=("bottom",))
    ...
    F.hgrid(ax)
    F.despine(ax)
    F.panel(ax, "b", "an assembly of each species resolves the 15 proteomes S3 found nothing in")
